...
void ModeThrow::run()
{
    ...
    Throw_Uprighting - the throw has been detected and the plopter is being uprighted
    Throw_HgtStabilise - the plopter is kept level and  height is stabilised about the target height
    Throw_PosHold - the plopter is kept at a constant position and height
    */
    ...
    if ((stage != prev_stage) || (now - last_log_ms) > 100) {
        ...
        const float accel = plopter.ins.get_accel().length();
...
// @URL: https://ardupilot.org/plopter/docs/throw-mode.html
...
bool ModeThrow::throw_detected()
{
    ...
    // Check if the accel length is < 1.0g indicating that any throw action is complete and the plopter has been released
    bool no_throw_action = plopter.ins.get_accel().length() < 1.0f * GRAVITY_MSS;
...
{
    // Check that we have uprighted the plopter
