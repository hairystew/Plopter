...
// arm_motors_check - checks for pilot input to arm or disarm the plopter
// called at 10hz
void Plopter::arm_motors_check()
{
...
// auto_disarm_check - disarms the plopter if it has been sitting on the ground in manual mode with throttle low for at least 15 seconds
void Plopter::auto_disarm_check()
{
    ...
    if ((ap.using_interlock && !motors->get_interlock()) || SRV_Channels::get_emergency_stop()) {
        ...
        // obvious the plopter is armed as the motors will not be spinning
        disarm_delay_ms /= 2;
#endif
    } else {
...
void Plopter::lost_vehicle_check()
{
    ...
    if (ap.throttle_zero && !motors->armed() && (channel_roll->get_control_in() > 4000) && (channel_pitch->get_control_in() > 4000)) {
        if (soundalarm_counter >= LOST_VEHICLE_DELAY) {
            if (AP_Notify::flags.vehicle_lost == false) {
                ...
                gcs().send_text(MAV_SEVERITY_NOTICE,"Locate Plopter alarm");
            }
        } else {
            soundalarm_counter++;
        }
