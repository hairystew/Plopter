...
bool ModeAuto::init(bool ignore_checks)
{
    ...
    if (mission.num_commands() > 1 || ignore_checks) {
        ...
        if (motors->armed() && plopter.ap.land_complete && !mission.starts_with_takeoff_cmd()) {
        ...
        // clear guided limits
        plopter.mode_guided.limit_clear();

        // reset flag indicating if pilot has applied roll or pitch inputs during landing
        plopter.ap.land_repo_active = false;

#if PRECISION_LANDING == ENABLED
        // initialise precland state machine
        plopter.precland_statemachine.init();
#endif
...
void ModeAuto::exit()
{
    if (plopter.mode_auto.mission.state() == AP_Mission::MISSION_RUNNING) {
        plopter.mode_auto.mission.stop();
    }
#if HAL_MOUNT_ENABLED
    plopter.camera_mount.set_mode_to_default();
...
void ModeAuto::run()
{
    ...
    if (waiting_to_start) {
        ...
        Location loc;
        if (plopter.ahrs.get_origin(loc)) {
    ...
    if (auto_RTL && (!(mission.get_in_landing_sequence_flag() || mission.state() == AP_Mission::mission_state::MISSION_COMPLETE))) {
        ...
        plopter.logger.Write_Mode((uint8_t)plopter.flightmode->mode_number(), ModeReason::AUTO_RTL_EXIT);
    }
}
...
void ModeAuto::set_submode(SubMode new_submode)
{
    ...
    if (old_submode == SubMode::NAV_ATTITUDE_TIME) {
        plopter.failsafe_ekf_recheck();
    }
}

bool ModeAuto::allows_arming(AP_Arming::Method method) const
{
    return ((plopter.g2.auto_options & (uint32_t)Options::AllowArming) != 0) && !auto_RTL;
};
...
bool ModeAuto::jump_to_landing_sequence_auto_RTL(ModeReason reason)
{
    if (mission.jump_to_landing_sequence()) {
        ...
        if ((plopter.flightmode == &plopter.mode_auto) || set_mode(Mode::Number::AUTO, reason)) {
            auto_RTL = true;
            // log entry into AUTO RTL
            plopter.logger.Write_Mode((uint8_t)plopter.flightmode->mode_number(), reason);
...
{
    if (!plopter.current_loc.initialised()) {
    ...
    if ((dest_loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) && wp_nav->get_terrain_offset(terrain_offset)) {
        ...
        dest.lat = plopter.current_loc.lat;
        dest.lng = plopter.current_loc.lng;
    ...
    // sanity check target
    int32_t alt_target_min_cm = current_alt_cm + (plopter.ap.land_complete ? 100 : 0);
...
void ModeAuto::wp_start(const Location& dest_loc)
{
    ...
    if (!wp_nav->set_wp_destination_loc(dest_loc)) {
        ...
        plopter.failsafe_terrain_on_event();
        return;
    }
...
void ModeAuto::land_start()
{
    ...
    plopter.landinggear.deploy_for_landing();
#endif
    ...
    plopter.ap.prec_land_active = false;
...
{
    // set circle center
    plopter.circle_nav->set_center(circle_center);

    // set circle radius
    if (!is_zero(radius_m)) {
        plopter.circle_nav->set_radius_cm(radius_m * 100.0f);
    }

    // check our distance from edge of circle
    Vector3f circle_edge_neu;
    plopter.circle_nav->get_closest_point_on_circle(circle_edge_neu);
    ...
    if (dist_to_edge > 300.0f) {
        ...
        const float dist_to_center = get_horizontal_distance_cm(inertial_nav.get_position_xy_cm().topostype(), plopter.circle_nav->get_center().xy());
        // initialise yaw
        // To-Do: reset the yaw only when the previous navigation command is not a WP.  this would allow removing the special check for ROI
        if (auto_yaw.mode() != AUTO_YAW_ROI) {
            if (dist_to_center > plopter.circle_nav->get_radius() && dist_to_center > 500) {
                auto_yaw.set_mode_to_default(false);
            } else {
...
void ModeAuto::circle_start()
{
    ...
    plopter.circle_nav->init(plopter.circle_nav->get_center(), plopter.circle_nav->center_is_terrain_alt());
...
void ModeAuto::nav_guided_start()
{
    ...
    if (!plopter.mode_guided.init(true)) {
    ...
    plopter.mode_guided.limit_init_time_and_pos();
...
bool ModeAuto::is_landing() const
{
    switch(_mode) {
        case SubMode::LAND:
            return true;
        case SubMode::RTL:
            return plopter.mode_rtl.is_landing();
        default:
            return false;
    }
    return false;
...
{
    return (plopter.g2.auto_options.get() & uint32_t(Options::IgnorePilotYaw)) == 0;
}
...
bool ModeAuto::start_command(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (plopter.should_log(MASK_LOG_CMD)) {
        plopter.logger.Write_Mission_Cmd(mission, cmd);
    }
    ...
    switch(cmd.id) {
        ...
        case MAV_CMD_DO_SET_ROI:                // 201
            // point the plopter and camera at a region of interest (ROI)
            do_roi(cmd);
            break;
...
void ModeAuto::exit_mission()
{
    ...
    if (!plopter.ap.land_complete) {
        ...
        plopter.arming.disarm(AP_Arming::Method::MISSIONEXIT);
    }
}
...
bool ModeAuto::do_guided(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (plopter.flightmode->mode_number() != Mode::Number::GUIDED && !(plopter.flightmode->mode_number() == Mode::Number::AUTO && _mode == SubMode::NAVGUIDED)) {
        return false;
    }
    ...
    switch (cmd.id) {
        ...
        case MAV_CMD_NAV_WAYPOINT:
        {
            ...
            return plopter.mode_guided.set_destination(dest);
        }
...
bool ModeAuto::get_wp(Location& destination) const
{
    switch (_mode) {
        case SubMode::NAVGUIDED:
            return plopter.mode_guided.get_wp(destination);
        case SubMode::WP:
            return wp_nav->get_oa_wp_destination(destination);
        case SubMode::RTL:
            return plopter.mode_rtl.get_wp(destination);
        default:
            return false;
    }
}
...
{
    if (plopter.flightmode != &plopter.mode_auto) {
        return false;
    }
...
void ModeAuto::takeoff_run()
{
    ...
    if ((plopter.g2.auto_options & (int32_t)Options::AllowTakeOffWithoutRaisingThrottle) != 0) {
...
void ModeAuto::wp_run()
{
    ...
    if (!plopter.failsafe.radio && use_pilot_yaw()) {
    ...
    plopter.failsafe_terrain_set_status(wp_nav->update_wpnav());
...
void ModeAuto::circle_run()
{
    ...
    plopter.failsafe_terrain_set_status(plopter.circle_nav->update());
    ...
    if (auto_yaw.mode() == AUTO_YAW_HOLD) {
        ...
        // roll, pitch from waypoint controller, yaw heading from auto_heading()
        attitude_control->input_thrust_vector_heading(plopter.circle_nav->get_thrust_vector(), auto_yaw.yaw());
    }
}
...
void ModeAuto::loiter_to_alt_run()
{
    ...
    const float alt_error_cm = plopter.current_loc.alt - loiter_to_alt.alt;
...
void ModeAuto::nav_attitude_time_run()
{
    ...
    const float angle_limit_cd = MAX(1000.0f, MIN(plopter.aparm.angle_max, attitude_control->get_althold_lean_angle_max_cd()));
...
Location ModeAuto::terrain_adjusted_location(const AP_Mission::Mission_Command& cmd) const
{
    ...
    if (plopter.current_loc.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, curr_terr_alt_cm) &&
        ...
        target_loc.set_alt_cm(plopter.current_loc.alt, Location::AltFrame::ABOVE_HOME);
    }
    return target_loc;
}
...
void ModeAuto::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    Location default_loc = plopter.current_loc;
...
void ModeAuto::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (target_loc.lat == 0 && target_loc.lng == 0) {
        ...
        Vector3f temp_pos;
        plopter.wp_nav->get_wp_stopping_point_xy(temp_pos.xy());
    ...
    if (target_loc.alt == 0) {
        ...
        int32_t curr_alt;
        if (plopter.current_loc.get_alt_cm(target_loc.get_alt_frame(),curr_alt)) {
            target_loc.set_alt_cm(curr_alt, target_loc.get_alt_frame());
        } else {
            // default to current altitude as alt-above-home
            target_loc.set_alt_cm(plopter.current_loc.alt,
                                  plopter.current_loc.get_alt_frame());
        }
    }
...
{
    const Location circle_center = loc_from_cmd(cmd, plopter.current_loc);
...
void ModeAuto::do_loiter_to_alt(const AP_Mission::Mission_Command& cmd)
{
    ...
    Location target_loc(cmd.content.location);
    if (target_loc.lat == 0 && target_loc.lng == 0) {
        target_loc.lat = plopter.current_loc.lat;
        target_loc.lng = plopter.current_loc.lng;
    }
...
void ModeAuto::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.content.speed.target_ms > 0) {
        if (cmd.content.speed.speed_type == 2)  {
            plopter.wp_nav->set_speed_up(cmd.content.speed.target_ms * 100.0f);
        } else if (cmd.content.speed.speed_type == 3)  {
            plopter.wp_nav->set_speed_down(cmd.content.speed.target_ms * 100.0f);
        } else {
            plopter.wp_nav->set_speed_xy(cmd.content.speed.target_ms * 100.0f);
        }
    }
}
...
void ModeAuto::do_set_home(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.p1 == 1 || (cmd.content.location.lat == 0 && cmd.content.location.lng == 0 && cmd.content.location.alt == 0)) {
        ...
        if (!plopter.set_home(cmd.content.location, false)) {
            // ignore failure
        }
    }
}
...
//          and possibly rotating the plopter to point at the ROI if our mount type does not support a yaw feature
...
void ModeAuto::do_mount_control(const AP_Mission::Mission_Command& cmd)
{
    ...
    plopter.camera_mount.set_angle_targets(cmd.content.mount_control.roll, cmd.content.mount_control.pitch, cmd.content.mount_control.yaw);
#endif
}
...
bool ModeAuto::verify_takeoff()
{
    ...
    if (auto_takeoff_complete) {
        ...
        plopter.landinggear.retract_after_takeoff();
    }
#endif
...
bool ModeAuto::verify_land()
{
    ...
    switch (state) {
        case State::FlyToLocation:
            ...
            if (plopter.wp_nav->reached_wp_destination()) {
                // initialise landing controller
                land_start();
        ...
        case State::Descending:
            ...
            retval = plopter.ap.land_complete && (motors->get_spool_state() == AP_Motors::SpoolState::GROUND_IDLE);
            if (retval && !mission.continue_after_land_check_for_takeoff() && plopter.motors->armed()) {
                /*
...
{
    return (plopter.mode_rtl.state_complete() &&
            (plopter.mode_rtl.state() == ModeRTL::SubMode::FINAL_DESCENT || plopter.mode_rtl.state() == ModeRTL::SubMode::LAND) &&
...
bool ModeAuto::verify_circle(const AP_Mission::Mission_Command& cmd)
{
    ...
    return fabsf(plopter.circle_nav->get_angle_total()/float(M_2PI)) >= LOWBYTE(cmd.p1);
}
...
bool ModeAuto::verify_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
    ...
    return plopter.mode_guided.limit_check();
}
#endif  // NAV_GUIDED
