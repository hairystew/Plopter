...
Mode::Mode(void) :
    g(plopter.g),
    g2(plopter.g2),
    wp_nav(plopter.wp_nav),
    loiter_nav(plopter.loiter_nav),
    pos_control(plopter.pos_control),
    inertial_nav(plopter.inertial_nav),
    ahrs(plopter.ahrs),
    attitude_control(plopter.attitude_control),
    motors(plopter.motors),
    channel_roll(plopter.channel_roll),
    channel_pitch(plopter.channel_pitch),
    channel_throttle(plopter.channel_throttle),
    channel_yaw(plopter.channel_yaw),
    G_Dt(plopter.G_Dt)
{ };

// return the static controller object corresponding to supplied mode
Mode *Plopter::mode_from_mode_num(const Mode::Number mode)
{
    Mode *ret = nullptr;
...
void Plopter::mode_change_failed(const Mode *mode, const char *reason)
{
...
bool Plopter::set_mode(Mode::Number mode, ModeReason reason)
{
    // update last reason
    ...
    if (mode == flightmode->mode_number()) {
        ...
        // make happy noise
        if (plopter.ap.initialised && (reason != last_reason)) {
    ...
    if (!ignore_checks &&
        ap.land_complete &&
        user_throttle &&
        !plopter.flightmode->has_manual_throttle() &&
        new_flightmode->get_pilot_desired_throttle() > plopter.get_non_takeoff_throttle()) {
...
bool Plopter::set_mode(const uint8_t new_mode, const ModeReason reason)
{
    ...
    if (reason == ModeReason::GCS_COMMAND && plopter.failsafe.radio) {
        // don't allow mode changes while in radio failsafe
        return false;
    }
#endif
    return plopter.set_mode(static_cast<Mode::Number>(new_mode), reason);
}

// update_flight_mode - calls the appropriate attitude controllers based on flight mode
// called at 100hz or more
void Plopter::update_flight_mode()
{
...
void Plopter::exit_mode(Mode *&old_flightmode,
                       Mode *&new_flightmode)
{
...
void Plopter::notify_flight_mode() {
...
void Mode::get_pilot_desired_lean_angles(float &roll_out, float &pitch_out, float angle_max, float angle_limit) const
{
    ...
    if (plopter.failsafe.radio || !plopter.ap.rc_receiver_present) {
        roll_out = 0;
        pitch_out = 0;
        return;
    }
...
Vector2f Mode::get_pilot_desired_velocity(float vel_max) const
{
    ...
    plopter.rotate_body_frame_to_NE(vel.x, vel.y);
...
bool Mode::_TakeOff::triggered(const float target_climb_rate) const
{
    ...
    if (plopter.motors->get_spool_state() != AP_Motors::SpoolState::THROTTLE_UNLIMITED) {
...
{
    if (!motors->armed() || !plopter.ap.auto_armed || plopter.ap.land_complete) {
        return true;
    }
    return false;
}
...
void Mode::zero_throttle_and_relax_ac(bool spool_up)
{
    ...
    attitude_control->set_throttle_out(0.0f, false, plopter.g.throttle_filt);
}
...
int32_t Mode::get_alt_above_ground_cm(void)
{
    ...
    if (plopter.get_rangefinder_height_interpolated_cm(alt_above_ground_cm)) {
        return alt_above_ground_cm;
    }
    if (!pos_control->is_active_xy()) {
        return plopter.current_loc.alt;
    }
    if (plopter.current_loc.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, alt_above_ground_cm)) {
        return alt_above_ground_cm;
    }
...
void Mode::land_run_vertical_control(bool pause_descent)
{
    ...
    if (!pause_descent) {
        ...
        ignore_descent_limit = (MAX(g2.land_alt_low,100) > get_alt_above_ground_cm()) || plopter.ap.land_complete_maybe;
        ...
        bool doing_precision_landing = !plopter.ap.land_repo_active && plopter.precland.target_acquired() && navigating;
        ...
        if (doing_precision_landing) {
            ...
            const float max_horiz_pos_error_cm = plopter.precland.get_max_xy_error_before_descending_cm();
            if (target_error_cm > max_horiz_pos_error_cm && !is_zero(max_horiz_pos_error_cm)) {
                ...
                // do not descend
                cmb_rate = 0.0f;
            } else if (plopter.rangefinder_alt_ok() && plopter.rangefinder_state.alt_cm > 35.0f && plopter.rangefinder_state.alt_cm < 200.0f) {
...
void Mode::land_run_horizontal_control()
{
    ...
    if (!plopter.failsafe.radio) {
        if ((g.throttle_behavior & THR_BEHAVE_HIGH_THROTTLE_CANCELS_LAND) != 0 && plopter.rc_throttle_control_in_filter.get() > LAND_CANCEL_TRIGGER_THR){
        ...
        if (g.land_repositioning) {
            ...
            if (!vel_correction.is_zero()) {
                if (!plopter.ap.land_repo_active) {
                    AP::logger().Write_Event(LogEvent::LAND_REPO_ACTIVE);
                }
                plopter.ap.land_repo_active = true;
            }
        }
...
#if PRECISION_LANDING == ENABLED
    plopter.ap.prec_land_active = !plopter.ap.land_repo_active && plopter.precland.target_acquired();
    // run precision landing
    if (plopter.ap.prec_land_active) {
    ...
    if (g2.wp_navalt_min > 0) {
        ...
        // over on landing, which particularly affects heliplopters if
        // there is any position estimate drift after touchdown. We
        // limit attitude to 7 degrees below this limit and linearly
        // interpolate for 1m above that
        const float attitude_limit_cd = linear_interpolate(700, plopter.aparm.angle_max, get_alt_above_ground_cm(),
...
{
#if PRECISION_LANDING == ENABLED
    if (pause_descent || !plopter.precland.enabled()) {
...
void Mode::precland_run()
{
    ...
    if (!plopter.ap.land_repo_active) {
        ...
        switch (plopter.precland_statemachine.update(retry_pos)) {
        ...
        case AC_PrecLand_StateMachine::Status::FAILSAFE: {
            ...
            switch (plopter.precland_statemachine.get_failsafe_actions()) {
...
float Mode::get_pilot_desired_throttle() const
{
    ...
    int16_t mid_stick = plopter.get_throttle_mid();
...
Mode::AltHoldModeState Mode::get_alt_hold_state(float target_climb_rate_cms)
{
    ...
    } else if (!plopter.ap.auto_armed || plopter.ap.land_complete) {
        // the aircraft is armed and landed
        if (target_climb_rate_cms < 0.0f && !plopter.ap.using_interlock) {
...
{
    return plopter.get_pilot_desired_climb_rate(throttle_control);
}

float Mode::get_non_takeoff_throttle()
{
    return plopter.get_non_takeoff_throttle();
}
...
{
    return plopter.set_mode(mode, reason);
}

void Mode::set_land_complete(bool b)
{
    return plopter.set_land_complete(b);
}

GCS_Plopter &Mode::gcs()
{
    return plopter.gcs();
}
...
{
    return plopter.get_pilot_speed_dn();
}
