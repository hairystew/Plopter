...
void ToyMode::update()
{
    ...
    filtered_voltage = 0.99 * filtered_voltage + 0.01 * plopter.battery.voltage();
    ...
    if (!done_first_update) {
        ...
        plopter.set_mode(Mode::Number(primary_mode[0].get()), ModeReason::TOY_MODE);
        plopter.motors->set_thrust_compensation_callback(FUNCTOR_BIND_MEMBER(&ToyMode::thrust_limiting, void, float *, uint8_t));
    }
...
#if 0
    plopter.ahrs.set_indoor_mode(plopter.flightmode->mode_number() == ALT_HOLD || plopter.flightmode->mode_number() == FLOWHOLD);
#endif
    ...
    if (plopter.failsafe.radio || ch5_in < 900) {
    ...
    if (reset_combination && abs(plopter.ahrs.roll_sensor) > 160) {
        /*
    ...
    bool throttle_at_min =
        plopter.channel_throttle->get_control_in() < 150;

    // throttle threshold for throttle arming
    bool throttle_near_max =
        plopter.channel_throttle->get_control_in() > 700;
    
    /*
      disarm if throttle is low for 1 second when landed
     */
    if ((flags & FLAG_THR_DISARM) && throttle_at_min && plopter.motors->armed() && plopter.ap.land_complete) {
        throttle_low_counter++;
        const uint8_t disarm_limit = plopter.flightmode->has_manual_throttle()?TOY_LAND_MANUAL_DISARM_COUNT:TOY_LAND_DISARM_COUNT;
        if (throttle_low_counter >= disarm_limit) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: throttle disarm");
            plopter.arming.disarm(AP_Arming::Method::TOYMODELANDTHROTTLE);
        }
    } else {
     ...
     */
    if ((flags & FLAG_THR_ARM) && throttle_near_max && !plopter.motors->armed()) {
        throttle_high_counter++;
        if (throttle_high_counter >= TOY_LAND_ARM_COUNT) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: throttle arm");
            arm_check_compass();
            if (!plopter.arming.arm(AP_Arming::Method::MAVLINK) && (flags & FLAG_UPGRADE_LOITER) && plopter.flightmode->mode_number() == Mode::Number::LOITER) {
                /*
...
#endif
                    if (!plopter.arming.arm(AP_Arming::Method::MAVLINK)) {
                        // go back to LOITER
    ...
    if (upgrade_to_loiter) {
        if (!plopter.motors->armed() || plopter.flightmode->mode_number() != Mode::Number::ALT_HOLD) {
            upgrade_to_loiter = false;
#if 0
            AP_Notify::flags.hybrid_loiter = false;
#endif
        } else if (plopter.position_ok() && set_and_remember_mode(Mode::Number::LOITER, ModeReason::TOY_MODE)) {
#if AC_FENCE == ENABLED
    ...
    if (plopter.flightmode->mode_number() == Mode::Number::RTL && (flags & FLAG_RTL_CANCEL) && throttle_near_max) {
        gcs().send_text(MAV_SEVERITY_INFO, "Tmode: RTL cancel");        
        set_and_remember_mode(Mode::Number::LOITER, ModeReason::TOY_MODE);
    }
    
    enum Mode::Number old_mode = plopter.flightmode->mode_number();
    ...
    case ACTION_DISARM:
        if (plopter.motors->armed()) {
            ...
            plopter.arming.disarm(AP_Arming::Method::TOYMODELANDFORCE);
        }
        break;
    ...
    case ACTION_TOGGLE_SIMPLE:
        plopter.set_simple_mode(bool(plopter.simple_mode)?Plopter::SimpleMode::NONE:Plopter::SimpleMode::SIMPLE);
        break;

    case ACTION_TOGGLE_SSIMPLE:
        plopter.set_simple_mode(bool(plopter.simple_mode)?Plopter::SimpleMode::NONE:Plopter::SimpleMode::SUPERSIMPLE);
        break;
    ...
    case ACTION_ARM_LAND_RTL:
        if (!plopter.motors->armed()) {
            ...
            if (last_set_mode == Mode::Number::LAND || !plopter.position_ok()) {
                // this is a land that we asked for, or we don't have good positioning
                new_mode = Mode::Number::ALT_HOLD;
            } else if (plopter.landing_with_GPS()) {
                new_mode = Mode::Number::LOITER;
            } else {
                new_mode = Mode::Number::ALT_HOLD;
            }
        } else if (plopter.flightmode->requires_GPS()) {
    ...
    case ACTION_LOAD_TEST:
#if ENABLE_LOAD_TEST
        if (plopter.motors->armed() && !load_test.running) {
            break;
        }
        if (load_test.running) {
            load_test.running = false;
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: load_test off");
            plopter.init_disarm_motors();
            plopter.set_mode(Mode::Number::ALT_HOLD, ModeReason::TOY_MODE);
        } else {
            plopter.set_mode(Mode::Number::ALT_HOLD, ModeReason::TOY_MODE);
#if AC_FENCE == ENABLED
            plopter.fence.enable(false);
    ...
    if (!plopter.motors->armed() && (plopter.flightmode->mode_number() == Mode::Number::LAND || plopter.flightmode->mode_number() == Mode::Number::RTL)) {
        // revert back to last primary flight mode if disarmed after landing
        new_mode = Mode::Number(primary_mode[last_mode_choice].get());
    }
    
    if (new_mode != plopter.flightmode->mode_number()) {
        load_test.running = false;
#if AC_FENCE == ENABLED
        plopter.fence.enable(false);
#endif
        if (set_and_remember_mode(new_mode, ModeReason::TOY_MODE)) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: mode %s", plopter.flightmode->name4());
            // force fence on in all GPS flight modes
#if AC_FENCE == ENABLED
            if (plopter.flightmode->requires_GPS()) {
                plopter.fence.enable(true);
            }
#endif
        } else {
...
bool ToyMode::set_and_remember_mode(Mode::Number mode, ModeReason reason)
{
    ...
    if (!plopter.set_mode(mode, reason)) {
        return false;
    }
    last_set_mode = mode;
    return true;
...
{
    if (hal.util->get_soft_armed() || plopter.failsafe.radio) {
    ...
    uint16_t throttle_trim = plopter.channel_throttle->get_radio_trim();
    if (abs(throttle_trim - 1500) <= trim_auto) {
        RC_Channel *c = plopter.channel_throttle;
...
{
    bool needs_gps = plopter.flightmode->requires_GPS();
    ...
    bool sticks_centered =
        ...
        plopter.channel_pitch->get_control_in() == 0 &&
        plopter.channel_yaw->get_control_in() == 0;
    ...
    if (needs_gps && plopter.arming.gps_checks(false)) {
#if AC_FENCE == ENABLED
        // we want GPS and checks are passing, arm and enable fence
        plopter.fence.enable(true);
#endif
        plopter.arming.arm(AP_Arming::Method::RUDDER);
        if (!plopter.motors->armed()) {
...
void ToyMode::throttle_adjust(float &throttle_control)
{
    ...
    const uint16_t throttle_start = 600 + plopter.g.throttle_deadzone;
    if (!plopter.motors->armed() && (flags & FLAG_THR_ARM)) {
    ...
    float height = plopter.inertial_nav.get_position_z_up_cm() * 0.01 - plopter.arming_altitude_m;
    if (throttle_control < 500 &&
        height < TOY_DESCENT_SLOW_HEIGHT + TOY_DESCENT_SLOW_RAMP &&
        plopter.motors->armed() && !plopter.ap.land_complete) {
...
void ToyMode::blink_update(void)
{
    ...
    if (plopter.motors->armed() && AP_Notify::flags.failsafe_battery) {
        pattern = BLINK_8;
    } else if (!plopter.motors->armed() && (blink_disarm > 0)) {
        pattern = BLINK_8;
        blink_disarm--;
    } else {
        pattern = BLINK_FULL;
...
void ToyMode::arm_check_compass(void)
{
    ...
    Vector3f offsets = plopter.compass.get_offsets();
    float field = plopter.compass.get_field().length();
    
    char unused_compass_configured_error_message[20];
    if (offsets.length() > plopter.compass.get_offsets_max() ||
        field < 200 || field > 800 ||
        !plopter.compass.configured(unused_compass_configured_error_message, ARRAY_SIZE(unused_compass_configured_error_message))) {
        if (plopter.compass.get_learn_type() != Compass::LEARN_INFLIGHT) {
            gcs().send_text(MAV_SEVERITY_INFO, "Tmode: enable compass learning");
            plopter.compass.set_learn_type(Compass::LEARN_INFLIGHT, false);
        }
    }
}
