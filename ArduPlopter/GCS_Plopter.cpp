...
void GCS_Plopter::update_vehicle_sensor_status_flags(void)
{
    ...
    if (plopter.have_reverse_thrust() && is_negative(SRV_Channels::get_output_scaled(SRV_Channel::k_throttle))) {
    ...
    case Mode::Number::TRAINING:
        if (!plopter.training_manual_roll || !plopter.training_manual_pitch) {
            rate_controlled = true;
            attitude_stabilized = true;
        }
        break;
    ...
    uint32_t last_valid = plopter.failsafe.last_valid_rc_ms;
    ...
    if (rangefinder && rangefinder->has_orientation(ROTATION_PITCH_270)) {
        ...
        if (plopter.g.rangefinder_landing) {
