 ...
 */
void Plopter::loiter_angle_reset(void)
{
    loiter.sum_cd = 0;
    loiter.total_cd = 0;
 ...
 */
void Plopter::loiter_angle_update(void)
{
...
void Plopter::navigate()
{
    ...
    // waypoint distance from plopter
    // ----------------------------
...
float Plopter::mode_auto_target_airspeed_cm()
{
    ...
    if (quadplopter.in_vtol_land_approach()) {
        return quadplopter.get_land_airspeed() * 100;
    }
#endif
...
void Plopter::calc_airspeed_errors()
{
...
#if HAL_QUADPLANE_ENABLED
    } else if (control_mode == &mode_qrtl && quadplopter.in_vtol_land_approach()) {
        target_airspeed_cm = quadplopter.get_land_airspeed() * 100;
#endif
    } else {
...
void Plopter::calc_gndspeed_undershoot()
{
    // Use the component of ground speed in the forward direction
    // This prevents flyaway if wind takes plopter backwards
...
void Plopter::update_loiter_update_nav(uint16_t radius)
{
#if HAL_QUADPLANE_ENABLED
    if (loiter.start_time_ms != 0 &&
        quadplopter.guided_mode_enabled()) {
...
#if HAL_QUADPLANE_ENABLED
    const bool quadplopter_qrtl_switch = (control_mode == &mode_rtl && quadplopter.available() && quadplopter.rtl_mode == QuadPlopter::RTL_MODE::SWITCH_QRTL);
#else
    const bool quadplopter_qrtl_switch = false;
#endif
    ...
    if ((loiter.start_time_ms == 0 &&
        ...
        quadplopter_qrtl_switch) {
        /*
          if never reached loiter point and using crosstrack and somewhat far away from loiter point
          navigate to it like in auto-mode for normal crosstrack behavior

          we also use direct waypoint navigation if we are a quadplopter
...
void Plopter::update_loiter(uint16_t radius)
{
    if (radius <= 1) {
...
#if HAL_QUADPLANE_ENABLED
            if (quadplopter.guided_mode_enabled()) {
                quadplopter.guided_start();
            }
#endif
        }
 ...
 */
void Plopter::update_fbwb_speed_height(void)
{
    uint32_t now = micros();
    if (now - target_altitude.last_elev_check_us >= 100000) {
        // we don't run this on every loop as it would give too small granularity on quadplopters at 300Hz, and
        ...
        if (g2.soaring_controller.is_active()) {
            if (g2.soaring_controller.get_throttle_suppressed()) {
                ...
                target_altitude.amsl_cm = 100*plopter.g2.soaring_controller.get_alt_cutoff() + 1000 + AP::ahrs().get_home().alt;
            }
        }
#endif
 ...
 */
void Plopter::setup_turn_angle(void)
{
 ...
 */
bool Plopter::reached_loiter_target(void)
{
#if HAL_QUADPLANE_ENABLED
