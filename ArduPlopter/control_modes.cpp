...
Mode *Plopter::mode_from_mode_num(const enum Mode::Number num)
{
    Mode *ret = nullptr;
    switch (num) {
...
void Plopter::read_control_switch()
{
...
uint8_t Plopter::readSwitch(void) const
{
...
void Plopter::reset_control_switch()
{
 ...
 */
void Plopter::autotune_start(void)
{
 ...
 */
void Plopter::autotune_restore(void)
{
 ...
 */
void Plopter::autotune_enable(bool enable)
{
    if (enable) {
        autotune_start();
    } else {
 ...
 */
bool Plopter::fly_inverted(void)
{
    if (control_mode == &plopter.mode_manual) {
        return false;
    }
    if (inverted_flight) {
