...
/********************************************************************************/
bool Plopter::start_command(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (AP_Mission::is_nav_cmd(cmd)) {
        ...
        if (have_next_cmd && quadplopter.is_vtol_land(next_nav_cmd.id)) {
...
#if HAL_QUADPLANE_ENABLED
        if (quadplopter.is_vtol_takeoff(cmd.id)) {
            return quadplopter.do_vtol_takeoff(cmd);
        }
#endif
        do_takeoff(cmd);
        break;
...
#if HAL_QUADPLANE_ENABLED
        if (quadplopter.is_vtol_land(cmd.id)) {
            crash_state.is_crashed = false;
            return quadplopter.do_vtol_land(cmd);
        }
#endif
        do_land(cmd);
        break;
    ...
    case MAV_CMD_NAV_VTOL_LAND:
        if (quadplopter.options & QuadPlopter::OPTION_MISSION_LAND_FW_APPROACH) {
            // the user wants to approach the landing in a fixed wing flight mode
            // the waypoint will be used as a loiter_to_alt
            // after which point the plopter will compute the optimal into the wind direction
            // and fly in on that direction towards the landing waypoint
            // it will then transition to VTOL and do a normal quadplopter landing
    ...
    case MAV_CMD_DO_VTOL_TRANSITION:
        plopter.quadplopter.handle_do_vtol_transition((enum MAV_VTOL_STATE)cmd.content.do_vtol_transition.target_state);
        break;
#endif

    case MAV_CMD_DO_ENGINE_CONTROL:
        plopter.g2.ice_control.engine_control(cmd.content.do_engine_control.start_control,
...
bool Plopter::verify_command(const AP_Mission::Mission_Command& cmd)        // Returns true if command complete
{
    switch(cmd.id) {
...
#if HAL_QUADPLANE_ENABLED
        if (quadplopter.is_vtol_takeoff(cmd.id)) {
            return quadplopter.verify_vtol_takeoff(cmd);
        }
#endif
...
#if HAL_QUADPLANE_ENABLED
        if (quadplopter.is_vtol_land(cmd.id)) {
            return quadplopter.verify_vtol_land();
        }
#endif
    ...
    case MAV_CMD_NAV_VTOL_TAKEOFF:
        return quadplopter.verify_vtol_takeoff(cmd);
    case MAV_CMD_NAV_VTOL_LAND:
        if ((quadplopter.options & QuadPlopter::OPTION_MISSION_LAND_FW_APPROACH) && !verify_landing_vtol_approach(cmd)) {
...
void Plopter::do_RTL(int32_t rtl_altitude_AMSL_cm)
{
 ...
 */
void Plopter::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
    prev_WP_loc = current_loc;
...
void Plopter::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    set_next_WP(cmd.content.location);
}

void Plopter::do_land(const AP_Mission::Mission_Command& cmd)
{
    set_next_WP(cmd.content.location);
...
#if HAL_QUADPLANE_ENABLED
void Plopter::do_landing_vtol_approach(const AP_Mission::Mission_Command& cmd)
{
    //set target alt
    ...
    // only set the direction if the quadplopter landing radius override is not 0
    // if it's 0 update_loiter will manage the direction for us when we hand it
    // 0 later in the controller
    if (is_negative(quadplopter.fw_land_approach_radius)) {
        loiter.direction = -1;
    } else if (is_positive(quadplopter.fw_land_approach_radius)) {
        loiter.direction = 1;
    }
...
void Plopter::loiter_set_direction_wp(const AP_Mission::Mission_Command& cmd)
{
...
void Plopter::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
...
void Plopter::do_loiter_turns(const AP_Mission::Mission_Command& cmd)
{
    Location cmdloc = cmd.content.location;
    cmdloc.sanitize(current_loc);
    set_next_WP(cmdloc);
...
void Plopter::do_loiter_time(const AP_Mission::Mission_Command& cmd)
{
    Location cmdloc = cmd.content.location;
    cmdloc.sanitize(current_loc);
    set_next_WP(cmdloc);
...
void Plopter::do_continue_and_change_alt(const AP_Mission::Mission_Command& cmd)
{
...
void Plopter::do_altitude_wait(const AP_Mission::Mission_Command& cmd)
{
    // set all servos to trim until we reach altitude or descent speed
    auto_state.idle_mode = true;
}

void Plopter::do_loiter_to_alt(const AP_Mission::Mission_Command& cmd)
{
    //set target alt  
    Location loc = cmd.content.location;
    loc.sanitize(current_loc);
    ...
    if (takeoff_state.start_time_ms != 0 && g2.takeoff_timeout > 0) {
        ...
        if (ground_speed >= takeoff_min_ground_speed) {
            ...
            if (now - takeoff_state.start_time_ms > (uint32_t)(1000U * g2.takeoff_timeout)) {
                ...
                plopter.arming.disarm(AP_Arming::Method::TAKEOFFTIMEOUT);
                mission.reset();
            }
        }
    }
 ...
 */
bool Plopter::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    steer_state.hold_course_cd = -1;
...
bool Plopter::verify_loiter_unlim(const AP_Mission::Mission_Command &cmd)
{
...
bool Plopter::verify_loiter_time()
{
    bool result = false;
...
bool Plopter::verify_loiter_turns(const AP_Mission::Mission_Command &cmd)
{
    bool result = false;
 ...
 */
bool Plopter::verify_loiter_to_alt(const AP_Mission::Mission_Command &cmd)
{
    bool result = false;
...
bool Plopter::verify_continue_and_change_alt()
{
 ...
 */
bool Plopter::verify_altitude_wait(const AP_Mission::Mission_Command &cmd)
{
...
void Plopter::do_wait_delay(const AP_Mission::Mission_Command& cmd)
{
    condition_start = millis();
    condition_value  = cmd.content.delay.seconds * 1000;    // convert seconds to milliseconds
}

void Plopter::do_within_distance(const AP_Mission::Mission_Command& cmd)
{
...
bool Plopter::verify_wait_delay()
{
...
bool Plopter::verify_within_distance()
{
...
void Plopter::do_loiter_at_location()
{
    if (aparm.loiter_radius < 0) {
        loiter.direction = -1;
    } else {
...
bool Plopter::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
...
void Plopter::do_set_home(const AP_Mission::Mission_Command& cmd)
{
...
bool Plopter::start_command_callback(const AP_Mission::Mission_Command &cmd)
{
...
//      we double check that the flight mode is AUTO to avoid the possibility of ap-mission triggering actions while we're not in AUTO mode
bool Plopter::verify_command_callback(const AP_Mission::Mission_Command& cmd)
{
    if (control_mode == &mode_auto) {
...
//      we double check that the flight mode is AUTO to avoid the possibility of ap-mission triggering actions while we're not in AUTO mode
void Plopter::exit_mission_callback()
{
    if (control_mode == &mode_auto) {
...
#if HAL_QUADPLANE_ENABLED
bool Plopter::verify_landing_vtol_approach(const AP_Mission::Mission_Command &cmd)
{
    switch (vtol_approach_s.approach_stage) {
        case RTL:
            {
                // fly home and loiter at RTL alt
                update_loiter(fabsf(quadplopter.fw_land_approach_radius));
                if (plopter.reached_loiter_target()) {
                    // decend to Q RTL alt
                    plopter.do_RTL(plopter.home.alt + plopter.quadplopter.qrtl_alt*100UL);
        ...
        case ENSURE_RADIUS:
            {
                float radius;
                if (is_zero(quadplopter.fw_land_approach_radius)) {
                    radius = aparm.loiter_radius;
                } else {
                    radius = quadplopter.fw_land_approach_radius;
                }
        ...
        case WAIT_FOR_BREAKOUT:
            {
                float radius = quadplopter.fw_land_approach_radius;
                if (is_zero(radius)) {
                    radius = aparm.loiter_radius;
                }
                const int8_t direction = is_negative(radius) ? -1 : 1;
        ...
        case APPROACH_LINE:
            {
                ...
                breakout_loc.offset_bearing(vtol_approach_s.approach_direction_deg + 180, quadplopter.stopping_distance());
                if(current_loc.past_interval_finish_line(start, breakout_loc)) {
                    vtol_approach_s.approach_stage = VTOL_LANDING;
                    quadplopter.do_vtol_land(cmd);
                    // fallthrough
                } else {
                    break;
                }
        ...
        case VTOL_LANDING:
            // nothing to do here, we should be into the quadplopter landing code
            return true;
    }
...
bool Plopter::verify_loiter_heading(bool init)
{
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_vtol_auto()) {
    ...
    return plopter.mode_loiter.isHeadingLinedUp(next_WP_loc, next_nav_cmd.content.location);
}

float Plopter::get_wp_radius() const
{
#if HAL_QUADPLANE_ENABLED
    if (plopter.quadplopter.in_vtol_mode()) {
        return plopter.quadplopter.wp_nav->get_wp_radius_cm() * 0.01;
    }
#endif
 ...
 */
void Plopter::do_nav_script_time(const AP_Mission::Mission_Command& cmd)
{
    ...
    nav_scripting.roll_rate_dps = plopter.rollController.get_pid_info().target;
    nav_scripting.pitch_rate_dps = plopter.pitchController.get_pid_info().target;
 ...
 */
bool Plopter::verify_nav_script_time(const AP_Mission::Mission_Command& cmd)
{
...
bool Plopter::nav_scripting_active(void) const
{
    return !nav_scripting.done &&
...
bool Plopter::nav_script_time(uint16_t &id, uint8_t &cmd, float &arg1, float &arg2)
{
...
void Plopter::nav_script_time_done(uint16_t id)
{
...
void Plopter::set_target_throttle_rate_rpy(float throttle_pct, float roll_rate_dps, float pitch_rate_dps, float yaw_rate_dps)
{
...
bool Plopter::nav_scripting_enable(uint8_t mode)
{
