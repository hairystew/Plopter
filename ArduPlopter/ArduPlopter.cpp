...
#define SCHED_TASK(func, rate_hz, max_time_micros, priority) SCHED_TASK_CLASS(Plopter, &plopter, func, rate_hz, max_time_micros, priority)
...
const AP_Scheduler::Task Plopter::scheduler_tasks[] = {
    ...
    SCHED_TASK_CLASS(GCS,            (GCS*)&plopter._gcs,       update_receive,   300,  500,  57),
    SCHED_TASK_CLASS(GCS,            (GCS*)&plopter._gcs,       update_send,      300,  750,  60),
    SCHED_TASK_CLASS(AP_ServoRelayEvents, &plopter.ServoRelayEvents, update_events, 50, 150,  63),
    SCHED_TASK_CLASS(AP_BattMonitor, &plopter.battery, read,   10, 300,  66),
    SCHED_TASK_CLASS(AP_Baro, &plopter.barometer, accumulate,  50, 150,  69),
    SCHED_TASK_CLASS(AP_Notify,      &plopter.notify,  update, 50, 300,  72),
#if AC_FENCE == ENABLED
    SCHED_TASK_CLASS(AC_Fence,       &plopter.fence,   update, 10, 100, 75),
#endif
    SCHED_TASK(read_rangefinder,       50,    100, 78),
    SCHED_TASK_CLASS(AP_ICEngine,      &plopter.g2.ice_control, update,     10, 100,  81),
    SCHED_TASK_CLASS(Compass,          &plopter.compass,        cal_update, 50,  50,  84),
#if AP_OPTICALFLOW_ENABLED
    SCHED_TASK_CLASS(OpticalFlow, &plopter.optflow, update,    50,    50,  87),
#endif
    SCHED_TASK(one_second_loop,         1,    400,  90),
    SCHED_TASK(three_hz_loop,           3,     75,  93),
    SCHED_TASK(check_long_failsafe,     3,    400,  96),
    SCHED_TASK_CLASS(AP_RPM,           &plopter.rpm_sensor,     update,     10, 100,  99),
#if AP_AIRSPEED_AUTOCAL_ENABLE
    SCHED_TASK(airspeed_ratio_update,   1,    100,  102),
#endif // AP_AIRSPEED_AUTOCAL_ENABLE
#if HAL_MOUNT_ENABLED
    SCHED_TASK_CLASS(AP_Mount, &plopter.camera_mount, update, 50, 100, 105),
#endif // HAL_MOUNT_ENABLED
#if CAMERA == ENABLED
    SCHED_TASK_CLASS(AP_Camera, &plopter.camera, update,      50, 100, 108),
#endif // CAMERA == ENABLED
    SCHED_TASK_CLASS(AP_Scheduler, &plopter.scheduler, update_logging,         0.2,    100, 111),
    ...
    SCHED_TASK_CLASS(AP_Terrain, &plopter.terrain, update, 10, 200, 132),
#endif // AP_TERRAIN_AVAILABLE
    SCHED_TASK(update_is_flying_5Hz,    5,    100, 135),
#if LOGGING_ENABLED == ENABLED
    SCHED_TASK_CLASS(AP_Logger,         &plopter.logger, periodic_tasks, 50, 400, 138),
#endif
    SCHED_TASK_CLASS(AP_InertialSensor, &plopter.ins,    periodic,       50,  50, 141),
#if HAL_ADSB_ENABLED
    SCHED_TASK(avoidance_adsb_update,  10,    100, 144),
#endif
    SCHED_TASK_CLASS(RC_Channels,       (RC_Channels*)&plopter.g2.rc_channels, read_aux_all,           10,    200, 147),
#if HAL_BUTTON_ENABLED
    SCHED_TASK_CLASS(AP_Button, &plopter.button, update, 5, 100, 150),
#endif
#if STATS_ENABLED == ENABLED
    SCHED_TASK_CLASS(AP_Stats, &plopter.g2.stats, update, 1, 100, 153),
#endif
#if GRIPPER_ENABLED == ENABLED
    SCHED_TASK_CLASS(AP_Gripper, &plopter.g2.gripper, update, 10, 75, 156),
#endif
...
void Plopter::get_scheduler_tasks(const AP_Scheduler::Task *&tasks,
                                uint8_t &task_count,
                                uint32_t &log_bit)
{
...
#else
constexpr int8_t Plopter::_failsafe_priorities[6];
#endif

// update AHRS system
void Plopter::ahrs_update()
{
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.tailsitter.active()) {
        rotate_limits = false;
    }
#endif
    if (rotate_limits) {
    ...
    quadplopter.check_yaw_reset();

    // update inertial_nav for quadplopter
    quadplopter.inertial_nav.update();
#endif
 ...
 */
void Plopter::update_speed_height(void)
{
    ...
    if (quadplopter.in_vtol_mode() ||
        quadplopter.in_assisted_flight()) {
        quadplopter.update_throttle_mix();
    }
#endif
}
 ...
 */
void Plopter::update_compass(void)
{
    compass.read();
}
...
/*
  do 10Hz logging
 */
void Plopter::update_logging1(void)
{
 ...
 */
void Plopter::update_logging2(void)
{
 ...
 */
void Plopter::airspeed_ratio_update(void)
{
    if (!airspeed.enabled() ||
        gps.status() < AP_GPS::GPS_OK_FIX_3D ||
        gps.ground_speed() < 4) {
 ...
 */
void Plopter::update_GPS_50Hz(void)
{
    gps.update();
 ...
 */
void Plopter::update_GPS_10Hz(void)
{
 ...
 */
void Plopter::update_control_mode(void)
{
...
void Plopter::update_fly_forward(void)
{
    ...
    if (quadplopter.available() &&
        quadplopter.tailsitter.is_in_fw_flight()) {
 ...
 */
void Plopter::set_flight_stage(AP_Vehicle::FixedWing::FlightStage fs)
{
    if (fs == flight_stage) {
        return;
    }
...
void Plopter::update_alt()
{
    barometer.update();

#if HAL_QUADPLANE_ENABLED
    if (quadplopter.available()) {
        quadplopter.motors->set_air_density_ratio(barometer.get_air_density_ratio());
    }
#endif
    ...
    if (control_mode->does_auto_throttle() && !throttle_suppressed) {
        ...
        if (control_mode == &mode_rtl && !rtl.done_climb && (g2.rtl_climb_min > 0 || (plopter.g2.flight_options & FlightOptions::CLIMB_BEFORE_TURN))) {
 ...
 */
void Plopter::update_flight_stage(void)
{
...
#if HAL_QUADPLANE_ENABLED
            if (quadplopter.in_assisted_flight()) {
                set_flight_stage(AP_Vehicle::FixedWing::FLIGHT_VTOL);
                return;
            }
#endif
 ...
 */
void Plopter::disarm_if_autoland_complete()
{
 ...
 */
float Plopter::tecs_hgt_afe(void)
{
    /*
...
bool Plopter::get_wp_distance_m(float &distance) const
{
    // see GCS_MAVLINK_Plopter::send_nav_controller_output()
    if (control_mode == &mode_manual) {
        return false;
    }
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_vtol_mode()) {
        distance = quadplopter.using_wp_nav() ? quadplopter.wp_nav->get_wp_distance_to_destination() * 0.01 : 0;
        return true;
    }
#endif
...
bool Plopter::get_wp_bearing_deg(float &bearing) const
{
    // see GCS_MAVLINK_Plopter::send_nav_controller_output()
    if (control_mode == &mode_manual) {
        return false;
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_vtol_mode()) {
        bearing = quadplopter.using_wp_nav() ? quadplopter.wp_nav->get_wp_bearing_to_destination() : 0;
        return true;
    }
#endif
...
bool Plopter::get_wp_crosstrack_error_m(float &xtrack_error) const
{
    // see GCS_MAVLINK_Plopter::send_nav_controller_output()
    if (control_mode == &mode_manual) {
        return false;
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_vtol_mode()) {
        xtrack_error = quadplopter.using_wp_nav() ? quadplopter.wp_nav->crosstrack_error() : 0;
        return true;
    }
#endif
...
bool Plopter::set_target_location(const Location &target_loc)
{
...
// set target location (for use by scripting)
bool Plopter::get_target_location(Location& target_loc)
{
 ...
 */
bool Plopter::update_target_location(const Location &old_loc, const Location &new_loc)
{
...
bool Plopter::set_velocity_match(const Vector2f &velocity)
{
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_vtol_mode() || quadplopter.in_vtol_land_sequence()) {
        quadplopter.poscontrol.velocity_match = velocity;
        quadplopter.poscontrol.last_velocity_match_ms = AP_HAL::millis();
        return true;
    }
#endif
    return false;
...
void Plopter::get_osd_roll_pitch_rad(float &roll, float &pitch) const
{
   pitch = ahrs.pitch;
   roll = ahrs.roll;
#if HAL_QUADPLANE_ENABLED
...
#if HAL_QUADPLANE_ENABLED
   pitch = quadplopter.ahrs_view->pitch;
   roll = quadplopter.ahrs_view->roll;
#endif
}
#endif

AP_HAL_MAIN_CALLBACKS(&plopter);
