...
void ModeAutorotate::run()
{
    ...
    if (motors->get_interlock() && !plopter.ap.land_complete) {
        phase_switch = Autorotation_Phase::BAIL_OUT;
    } else if (motors->get_interlock() && plopter.ap.land_complete) {
        // Aircraft is landed and no need to bail out
        set_mode(plopter.prev_control_mode, ModeReason::AUTOROTATION_BAILOUT);
    }
    ...
    switch (phase_switch) {
        ...
        if ((now - _bail_time_start_ms)/1000.0f >= _bail_time) {
            ...
            if (plopter.prev_control_mode == Mode::Number::AUTO) {
    ...
    switch (nav_pos_switch) {
        ...
        case Navigation_Decision::USER_CONTROL_STABILISED:
        {
            ...
            get_pilot_desired_lean_angles(pilot_roll, pilot_pitch, plopter.aparm.angle_max, plopter.aparm.angle_max);
