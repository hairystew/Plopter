...
bool ModeGuided::allows_arming(AP_Arming::Method method) const
{
    ...
    return (plopter.g2.guided_options & (uint32_t)Options::AllowArmingFromTX) != 0;
};
...
bool ModeGuided::do_user_takeoff_start(float takeoff_alt_cm)
{
    ...
    if (wp_nav->rangefinder_used_and_healthy() &&
        ...
        takeoff_alt_cm < plopter.rangefinder.max_distance_cm_orient(ROTATION_PITCH_270)) {
        // can't takeoff downwards
        if (takeoff_alt_cm <= plopter.rangefinder_state.alt_cm) {
            return false;
        }
        ...
        Location target_loc = plopter.current_loc;
...
bool ModeGuided::set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw, bool terrain_alt)
{
    ...
    if (!plopter.fence.check_destination_within_fence(dest_loc)) {
    ...
    if (use_wpnav_for_position_control()) {
        ...
        // log target
        plopter.Log_Write_Guided_Position_Target(guided_mode, destination, terrain_alt, Vector3f(), Vector3f());
    ...
    // log target
    plopter.Log_Write_Guided_Position_Target(guided_mode, guided_pos_target_cm.tofloat(), guided_pos_terrain_alt, guided_vel_target_cms, guided_accel_target_cmss);
...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    ...
    if (use_wpnav_for_position_control()) {
        ...
        // log target
        plopter.Log_Write_Guided_Position_Target(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), (dest_loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN), Vector3f(), Vector3f());
        send_notification = true;
        return true;
    }
    ...
    // log target
    plopter.Log_Write_Guided_Position_Target(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), guided_pos_terrain_alt, guided_vel_target_cms, guided_accel_target_cmss);
...
{
    return ((plopter.g2.guided_options.get() & uint32_t(Options::SetAttitudeTarget_ThrustAsThrust)) != 0);
}
...
{
    return !((plopter.g2.guided_options.get() & uint32_t(Options::DoNotStabilizePositionXY)) != 0);
}
...
{
    return !((plopter.g2.guided_options.get() & uint32_t(Options::DoNotStabilizeVelocityXY)) != 0);
}
...
{
    return ((plopter.g2.guided_options.get() & uint32_t(Options::WPNavUsedForPosControl)) != 0);
}
...
void ModeGuided::set_angle(const Quaternion &attitude_quat, const Vector3f &ang_vel, float climb_rate_cms_or_thrust, bool use_thrust)
{
    ...
    // log target
    plopter.Log_Write_Guided_Attitude_Target(guided_mode, roll_rad, pitch_rad, yaw_rad, ang_vel, guided_angle_state.thrust, guided_angle_state.climb_rate_cms * 0.01);
}
...
void ModeGuided::pos_control_run()
{
    ...
    if (guided_pos_terrain_alt) {
        pos_offset_z_buffer = MIN(plopter.wp_nav->get_terrain_margin() * 100.0, 0.5 * fabsf(guided_pos_target_cm.z));
    }
...
void ModeGuided::velaccel_control_run()
{
    ...
    plopter.avoid.adjust_velocity(guided_vel_target_cms, pos_control->get_pos_xy_p().kP(), pos_control->get_max_accel_xy_cmss(), pos_control->get_pos_z_p().kP(), pos_control->get_max_accel_z_cmss(), G_Dt);
    do_avoid = plopter.avoid.limits_active();
#endif
...
void ModeGuided::angle_control_run()
{
    ...
    // if not armed set throttle to zero and exit immediately
    if (!motors->armed() || !plopter.ap.auto_armed || (plopter.ap.land_complete && !positive_thrust_or_climbrate)) {
        // do not spool down tradheli when on the ground with motor interlock enabled
        make_safe_ground_handling(plopter.is_tradheli() && motors->get_interlock());
        return;
    }
    ...
    if (plopter.ap.land_complete && (guided_angle_state.climb_rate_cms > 0.0f)) {
    ...
    if (guided_angle_state.use_thrust) {
        attitude_control->set_throttle_out(guided_angle_state.thrust, true, plopter.g.throttle_filt);
    } else {
...
{
    return (plopter.g2.guided_options.get() & uint32_t(Options::IgnorePilotYaw)) == 0;
}
...
{
    return MAX(plopter.g2.guided_timeout, 0.1) * 1000;
}
