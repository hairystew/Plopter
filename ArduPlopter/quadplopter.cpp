...
const AP_Param::GroupInfo QuadPlopter::var_info[] = {

    // @Param: ENABLE
    // @DisplayName: Enable QuadPlopter
    // @Description: This enables QuadPlopter functionality, assuming multicopter motors start on output 5. If this is set to 2 then when starting AUTO mode it will initially be in VTOL AUTO mode.
    // @Values: 0:Disable,1:Enable,2:Enable VTOL AUTO
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO_FLAGS("ENABLE", 1, QuadPlopter, enable, 0, AP_PARAM_FLAG_ENABLE),

    // @Group: M_
    // @Path: ../libraries/AP_Motors/AP_MotorsMulticopter.cpp
    AP_SUBGROUPVARPTR(motors, "M_", 2, QuadPlopter, plopter.quadplopter.motors_var_info),

    // 3 ~ 8 were used by quadplopter attitude control PIDs
    ...
    // @Range: 1000 8000
    // @User: Advanced
    AP_GROUPINFO("ANGLE_MAX", 10, QuadPlopter, aparm.angle_max, 3000),
    ...
    // @Range: 1 30000
    // @User: Advanced
    AP_GROUPINFO("TRANSITION_MS", 11, QuadPlopter, transition_time_ms, 5000),

    // 12 ~ 16 were used by position, velocity and acceleration PIDs

    // @Group: P
    // @Path: ../libraries/AC_AttitudeControl/AC_PosControl.cpp
    AP_SUBGROUPPTR(pos_control, "P", 17, QuadPlopter, AC_PosControl),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("VELZ_MAX", 18, QuadPlopter, pilot_velocity_z_max_up, 250),
   ...
   // @Param: VELZ_MAX_DN
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("VELZ_MAX_DN", 60, QuadPlopter, pilot_velocity_z_max_dn, 0),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("ACCEL_Z",  19, QuadPlopter, pilot_accel_z,  250),

    // @Group: WP_
    // @Path: ../libraries/AC_WPNav/AC_WPNav.cpp
    AP_SUBGROUPPTR(wp_nav, "WP_",  20, QuadPlopter, AC_WPNav),

    // @Param: RC_SPEED
    // @DisplayName: RC output speed in Hz
    // @Description: This is the PWM refresh rate in Hz for QuadPlopter quad motors
    // @Units: Hz
    // @Range: 50 500
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("RC_SPEED", 21, QuadPlopter, rc_speed, 490),
    ...
    // @Param: ASSIST_SPEED
    // @DisplayName: Quadplopter assistance speed
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("ASSIST_SPEED", 24, QuadPlopter, assist_speed, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("YAW_RATE_MAX", 25, QuadPlopter, yaw_rate_max, 100),
    ...
    // @Increment: 10
    // @User: Standard
    AP_GROUPINFO("LAND_SPEED", 26, QuadPlopter, land_speed_cms, 50),
    ...
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("LAND_FINAL_ALT", 27, QuadPlopter, land_final_alt, 6),
    ...
    // @Units: deg
    // @Increment: 1
    AP_GROUPINFO("TRAN_PIT_MAX", 29, QuadPlopter, transition_pitch_max, 3),
...
#define FRAME_CLASS_OLD_IDX 30
    ...
    // @User: Standard
    AP_GROUPINFO("FRAME_CLASS", 46, QuadPlopter, frame_class, 1),
    ...
    // @User: Standard
    AP_GROUPINFO("FRAME_TYPE", 31, QuadPlopter, frame_type, 1),
    ...
    // @Increment: 0.01
    // @User: Standard
    AP_GROUPINFO("VFWD_GAIN", 32, QuadPlopter, vel_forward.gain, 0),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("RTL_ALT", 35, QuadPlopter, qrtl_alt, 15),
    ...
    // @User: Standard
    AP_GROUPINFO("RTL_MODE", 36, QuadPlopter, rtl_mode, 0),
    ...
    // @Increment: 0.25
    // @User: Standard
    AP_GROUPINFO("VFWD_ALT", 43, QuadPlopter, vel_forward_alt_cutoff,  0),
    ...
    AP_GROUPINFO("OPTIONS", 58, QuadPlopter, options, 0),

    AP_SUBGROUPEXTENSION("",59, QuadPlopter, var_info2),
...
// second table of user settable parameters for quadplopters, this
// allows us to go beyond the 64 parameter limit
const AP_Param::GroupInfo QuadPlopter::var_info2[] = {
    // @Param: TRANS_DECEL
    ...
    // @Range: 0.2 5
    // @User: Standard
    AP_GROUPINFO("TRANS_DECEL", 1, QuadPlopter, transition_decel, 2.0),

    // @Group: LOIT_
    // @Path: ../libraries/AC_WPNav/AC_Loiter.cpp
    AP_SUBGROUPPTR(loiter_nav, "LOIT_",  2, QuadPlopter, AC_Loiter),

    // 3: TAILSIT_GSCMAX

    // @Param: TRIM_PITCH
    // @DisplayName: Quadplopter AHRS trim pitch
    ...
    // @User: Advanced
    // @RebootRequired: True
    AP_GROUPINFO("TRIM_PITCH", 4, QuadPlopter, ahrs_trim_pitch, 0),
    ...
    AP_SUBGROUPINFO(qautotune, "AUTOTUNE_",  6, QuadPlopter, QAutoTune),
#endif

    // @Param: FW_LND_APR_RAD
    // @DisplayName: Quadplopter fixed wing landing approach radius
    ...
    // @Increment: 5
    // @User: Advanced
    AP_GROUPINFO("FW_LND_APR_RAD", 7, QuadPlopter, fw_land_approach_radius, 0),

    // @Param: TRANS_FAIL
    // @DisplayName: Quadplopter transition failure time
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("TRANS_FAIL", 8, QuadPlopter, transition_failure.timeout, 0),
    ...
    // @Increment: .1
    // @User: Advanced
    AP_GROUPINFO("THROTTLE_EXPO", 10, QuadPlopter, throttle_expo, 0.2),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("ACRO_RLL_RATE", 11, QuadPlopter, acro_roll_rate, 360),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("ACRO_PIT_RATE", 12, QuadPlopter, acro_pitch_rate, 180),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("ACRO_YAW_RATE", 13, QuadPlopter, acro_yaw_rate, 90),
    ...
    // @Increment: 5.1
    // @User: Advanced
    AP_GROUPINFO("TKOFF_FAIL_SCL", 14, QuadPlopter, takeoff_failure_scalar, 0),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("TKOFF_ARSP_LIM", 15, QuadPlopter, maximum_takeoff_airspeed, 0),

    // @Param: ASSIST_ALT
    // @DisplayName: Quadplopter assistance altitude
    // @Description: This is the altitude below which quadplopter assistance will be triggered. This acts the same way as Q_ASSIST_ANGLE and Q_ASSIST_SPEED, but triggers if the aircraft drops below the given altitude while the VTOL motors are not running. A value of zero disables this feature. The altutude is calculated as being above ground level. The height above ground is given from a Lidar used if available and RNGFND_LANDING=1. Otherwise it comes from terrain data if TERRAIN_FOLLOW=1 and comes from height above home otherwise.
    // @Units: m
    // @Range: 0 120
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("ASSIST_ALT", 16, QuadPlopter, assist_alt, 0),

    // 17: TAILSIT_GSCMSK
    // 18: TAILSIT_GSCMIN

    // @Param: ASSIST_DELAY
    // @DisplayName: Quadplopter assistance delay
    // @Description: This is delay between the assistance thresholds being met and the assistance starting.
    // @Units: s
    // @Range: 0 2
    // @Increment: 0.1
    // @User: Standard
    AP_GROUPINFO("ASSIST_DELAY", 19, QuadPlopter, assist_delay, 0.5),
    ...
    // @Range: 0 100
    AP_GROUPINFO("FWD_MANTHR_MAX", 20, QuadPlopter, fwd_thr_max, 0),
    ...
    // @Group: TAILSIT_
    // @Path: tailsitter.cpp
    AP_SUBGROUPINFO(tailsitter, "TAILSIT_", 26, QuadPlopter, Tailsitter),

    // @Group: TILT_
    // @Path: tiltrotor.cpp
    AP_SUBGROUPINFO(tiltrotor, "TILT_", 27, QuadPlopter, Tiltrotor),
    ...
    // @Units: ms
    // @Range: 0 10000
    AP_GROUPINFO("BACKTRANS_MS", 28, QuadPlopter, back_trans_pitch_limit_ms, 3000),

    // @Param: TRANS_FAIL_ACT
    // @DisplayName: Quadplopter transition failure action
    ...
    AP_GROUPINFO("TRANS_FAIL_ACT", 29, QuadPlopter, transition_failure.action, 0),

    // @Group: WVANE_
    // @Path: ../libraries/AC_AttitudeControl/AC_WeatherVane.cpp
    AP_SUBGROUPPTR(weathervane, "WVANE_", 30, QuadPlopter, AC_WeatherVane),
    ...
    // @Increment: 0.05
    // @User: Standard
    AP_GROUPINFO("LAND_ALTCHG", 31, QuadPlopter, landing_detect.detect_alt_change, 0.2),
    ...
    // @Range: 0 5
    // @User: Advanced
    AP_GROUPINFO("NAVALT_MIN", 32, QuadPlopter, takeoff_navalt_min, 0),
    
    AP_GROUPEND
};

/*
  defaults for all quadplopters
 */
...
/*
  conversion table for quadplopter parameters
 */
const AP_Param::ConversionInfo q_conversion_table[] = {
    { Parameters::k_param_quadplopter, 4044, AP_PARAM_FLOAT, "Q_P_POSZ_P" },     //  Q_PZ_P
    { Parameters::k_param_quadplopter, 4045, AP_PARAM_FLOAT, "Q_P_POSXY_P"},     //  Q_PXY_P
    { Parameters::k_param_quadplopter, 4046, AP_PARAM_FLOAT, "Q_P_VELXY_P"},     //  Q_VXY_P
    { Parameters::k_param_quadplopter, 78,   AP_PARAM_FLOAT, "Q_P_VELXY_I"},     //  Q_VXY_I
    { Parameters::k_param_quadplopter, 142,  AP_PARAM_FLOAT, "Q_P_VELXY_IMAX"},  //  Q_VXY_IMAX
    { Parameters::k_param_quadplopter, 206,  AP_PARAM_FLOAT, "Q_P_VELXY_FLTE"},  //  Q_VXY_FILT_HZ
    { Parameters::k_param_quadplopter, 4047, AP_PARAM_FLOAT, "Q_P_VELZ_P"},      //  Q_VZ_P
    { Parameters::k_param_quadplopter, 4048, AP_PARAM_FLOAT, "Q_P_ACCZ_P"},      //  Q_AZ_P
    { Parameters::k_param_quadplopter, 80,   AP_PARAM_FLOAT, "Q_P_ACCZ_I"},      //  Q_AZ_I
    { Parameters::k_param_quadplopter, 144,  AP_PARAM_FLOAT, "Q_P_ACCZ_D"},      //  Q_AZ_D
    { Parameters::k_param_quadplopter, 336,  AP_PARAM_FLOAT, "Q_P_ACCZ_IMAX"},   //  Q_AZ_IMAX
    { Parameters::k_param_quadplopter, 400,  AP_PARAM_FLOAT, "Q_P_ACCZ_FLTD"},   //  Q_AZ_FILT
    { Parameters::k_param_quadplopter, 464,  AP_PARAM_FLOAT, "Q_P_ACCZ_FF"},     //  Q_AZ_FF
    { Parameters::k_param_quadplopter, 276,  AP_PARAM_FLOAT, "Q_LOIT_SPEED"},    //  Q_WP_LOIT_SPEED
    { Parameters::k_param_quadplopter, 468,  AP_PARAM_FLOAT, "Q_LOIT_BRK_JERK" },//  Q_WP_LOIT_JERK
    { Parameters::k_param_quadplopter, 532,  AP_PARAM_FLOAT, "Q_LOIT_ACC_MAX" }, //  Q_WP_LOIT_MAXA
    { Parameters::k_param_quadplopter, 596,  AP_PARAM_FLOAT, "Q_LOIT_BRK_ACCEL" },// Q_WP_LOIT_MINA
    ...
    { Parameters::k_param_quadplopter, 48,  AP_PARAM_INT8,  "Q_TAILSIT_ANGLE" },
    { Parameters::k_param_quadplopter, 61,  AP_PARAM_INT8,  "Q_TAILSIT_ANG_VT" },
    { Parameters::k_param_quadplopter, 50,  AP_PARAM_INT8,  "Q_TAILSIT_INPUT" },
    { Parameters::k_param_quadplopter, 53,  AP_PARAM_FLOAT, "Q_TAILSIT_VFGAIN" },
    { Parameters::k_param_quadplopter, 54,  AP_PARAM_FLOAT, "Q_TAILSIT_VHGAIN" },
    { Parameters::k_param_quadplopter, 56,  AP_PARAM_FLOAT, "Q_TAILSIT_VHPOW" },
    { Parameters::k_param_quadplopter, 251,   AP_PARAM_FLOAT, "Q_TAILSIT_GSCMAX" },
    { Parameters::k_param_quadplopter, 379,   AP_PARAM_FLOAT, "Q_TAILSIT_RLL_MX" },
    { Parameters::k_param_quadplopter, 635,   AP_PARAM_INT16, "Q_TAILSIT_MOTMX" },
    { Parameters::k_param_quadplopter, 1147,  AP_PARAM_INT16, "Q_TAILSIT_GSCMSK" },
    { Parameters::k_param_quadplopter, 1211,  AP_PARAM_FLOAT, "Q_TAILSIT_GSCMIN" },
    { Parameters::k_param_quadplopter, 1403,  AP_PARAM_FLOAT, "Q_TAILSIT_DSKLD" },
    { Parameters::k_param_quadplopter, 1595,  AP_PARAM_FLOAT, "Q_TAILSIT_RAT_FW" },
    { Parameters::k_param_quadplopter, 1659,  AP_PARAM_FLOAT, "Q_TAILSIT_RAT_FW" },

    // tiltrotor params have moved but retain the same names
    { Parameters::k_param_quadplopter, 37,  AP_PARAM_INT16,  "Q_TILT_MASK" },
    { Parameters::k_param_quadplopter, 38,  AP_PARAM_INT16,  "Q_TILT_RATE_UP" },
    { Parameters::k_param_quadplopter, 39,  AP_PARAM_INT8,  "Q_TILT_MAX" },
    { Parameters::k_param_quadplopter, 47,  AP_PARAM_INT8,  "Q_TILT_TYPE" },
    { Parameters::k_param_quadplopter, 49,  AP_PARAM_INT16,  "Q_TILT_RATE_DN" },
    { Parameters::k_param_quadplopter, 55,  AP_PARAM_FLOAT,  "Q_TILT_YAW_ANGLE" },
    { Parameters::k_param_quadplopter, 1467,  AP_PARAM_FLOAT,  "Q_TILT_FIX_ANGLE" },
    { Parameters::k_param_quadplopter, 1531,  AP_PARAM_FLOAT,  "Q_TILT_FIX_GAIN" },

    // PARAMETER_CONVERSION - Added: Jan-2022
    { Parameters::k_param_quadplopter, 33,  AP_PARAM_FLOAT, "Q_WVANE_GAIN" },     // Moved from quadplopter to weathervane library
    { Parameters::k_param_quadplopter, 34,  AP_PARAM_FLOAT, "Q_WVANE_ANG_MIN" },  // Q_WVANE_MINROLL moved from quadplopter to weathervane library
};

// PARAMETER_CONVERSION - Added: Oct-2021
const AP_Param::ConversionInfo mot_pwm_conversion_table[] = {
    { Parameters::k_param_quadplopter, 22,  AP_PARAM_INT16, "Q_M_PWM_MIN" },
    { Parameters::k_param_quadplopter, 23,  AP_PARAM_INT16, "Q_M_PWM_MAX" },
};

QuadPlopter::QuadPlopter(AP_AHRS &_ahrs) :
    ahrs(_ahrs)
{
    ...
    if (_singleton != nullptr) {
        AP_HAL::panic("Can only be one Quadplopter");
    }
    _singleton = this;
}
...
void QuadPlopter::setup_default_channels(uint8_t num_motors)
{
...
bool QuadPlopter::setup(void)
{
    ...
    float loop_delta_t = 1.0 / plopter.scheduler.get_loop_rate_hz();

    /*
      cope with upgrade from old AP_Motors values for frame_class
     */
    AP_Int8 old_class;
    const AP_Param::ConversionInfo cinfo { Parameters::k_param_quadplopter, FRAME_CLASS_OLD_IDX, AP_PARAM_INT8, nullptr };
    ...
    if (hal.util->available_memory() <
        ...
        AP_BoardConfig::config_error("Not enough memory for quadplopter");
    }

    /*
      dynamically allocate the key objects for quadplopter. This ensures
    ...
    switch ((AP_Motors::motor_frame_class)frame_class) {
    case AP_Motors::MOTOR_FRAME_TRI:
        motors = new AP_MotorsTri(plopter.scheduler.get_loop_rate_hz(), rc_speed);
        motors_var_info = AP_MotorsTri::var_info;
        break;
    case AP_Motors::MOTOR_FRAME_TAILSITTER:
        // this is a duo-motor tailsitter
        tailsitter.tailsitter_motors = new AP_MotorsTailsitter(plopter.scheduler.get_loop_rate_hz(), rc_speed);
    ...
    case AP_Motors::MOTOR_FRAME_DYNAMIC_SCRIPTING_MATRIX:
            ...
            break;
    default:
        motors = new AP_MotorsMatrix(plopter.scheduler.get_loop_rate_hz(), rc_speed);
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "QuadPlopter initialised, %s", frame_and_type_string);
    initialised = true;
    return true;
}
 ...
 */
void QuadPlopter::setup_defaults(void)
{
    ...
    // Quadplopters need the same level of GPS error checking as Copters do, Plopter is more relaxed
...
// run ESC calibration
void QuadPlopter::run_esc_calibration(void)
{
    if (!motors->armed()) {
    ...
    case 1:
        ...
        motors->set_throttle_passthrough_for_esc_calibration(plopter.get_throttle_input() * 0.01f);
        break;
    case 2:
 ...
 */
void QuadPlopter::multicopter_attitude_rate_update(float yaw_rate_cds)
{
    ...
    if (use_multicopter_control) {
        ...
        if (tailsitter.enabled() &&
            ...
            if (!(tailsitter.input_type & Tailsitter::input::TAILSITTER_INPUT_PLANE)) {
                ...
                // In plopter input mode, the roll and yaw sticks are swapped
        ...
        if (use_yaw_target) {
            attitude_control->input_euler_angle_roll_pitch_yaw(plopter.nav_roll_cd,
                                                               plopter.nav_pitch_cd,
                                                               yaw_target_cd,
                                                               true);
        } else {
        ...
        float roll_rate = plopter.rollController.get_pid_info().target;
        float pitch_rate = plopter.pitchController.get_pid_info().target;
        if (tailsitter.enabled()) {
...
void QuadPlopter::hold_stabilize(float throttle_in)
{    
...
void QuadPlopter::run_z_controller(void)
{
...
void QuadPlopter::relax_attitude_control()
{
 ...
 */
void QuadPlopter::check_yaw_reset(void)
{
    if (!initialised) {
        return;
    }
...
void QuadPlopter::set_climb_rate_cms(float target_climb_rate_cms, bool force_descend)
{
 ...
 */
void QuadPlopter::hold_hover(float target_climb_rate_cms)
{
...
float QuadPlopter::get_pilot_throttle()
{
    // get scaled throttle input
    float throttle_in = plopter.channel_throttle->get_control_in();

    // normalize to [0,1]
    throttle_in /= plopter.channel_throttle->get_range();
...
*/
void QuadPlopter::get_pilot_desired_lean_angles(float &roll_out_cd, float &pitch_out_cd, float angle_max_cd, float angle_limit_cd) const
{
    // failsafe check
    if (plopter.failsafe.rc_failsafe || plopter.failsafe.throttle_counter > 0) {
        roll_out_cd = 0;
        pitch_out_cd = 0;
        return;
    }
    ...
    roll_out_cd = plopter.channel_roll->get_control_in();
    pitch_out_cd = plopter.channel_pitch->get_control_in();
...
*/
float QuadPlopter::get_pilot_land_throttle(void) const
{
    if (plopter.rc_failsafe_active()) {
...
bool QuadPlopter::is_flying(void)
{
    if (!available()) {
        return false;
    }
    if (plopter.control_mode == &plopter.mode_guided && guided_takeoff) {
        return true;
    }
...
bool QuadPlopter::should_relax(void)
{
...
bool QuadPlopter::is_flying_vtol(void) const
{
    if (!available()) {
        return false;
    }
    ...
    if (plopter.control_mode->is_vtol_man_throttle() && air_mode_active()) {
...
/*
  ...
  land_final_alt. 
 */
float QuadPlopter::landing_descent_rate_cms(float height_above_ground)
{
 ...
 */
float QuadPlopter::get_pilot_input_yaw_rate_cds(void) const
{
    const auto rudder_in = plopter.channel_rudder->get_control_in();
    bool manual_air_mode = plopter.control_mode->is_vtol_man_throttle() && air_mode_active();
    if (!manual_air_mode &&
        !is_positive(plopter.get_throttle_input()) &&
        (!plopter.control_mode->does_auto_throttle() || motors->limit.throttle_lower) &&
        plopter.arming.get_rudder_arming_type() == AP_Arming::RudderArming::ARMDISARM &&
        rudder_in < 0 &&
    ...
    if ((plopter.g.stick_mixing == StickMixing::NONE) &&
        (plopter.control_mode == &plopter.mode_qrtl ||
         plopter.control_mode->is_guided_mode() ||
         in_vtol_auto())) {
        return 0;
    }
    ...
    if (!in_vtol_mode() && tailsitter.enabled()) {
        ...
        // when hovering
        max_rate *= plopter.g2.rudd_dt_gain * 0.01;
    }
    if (tailsitter.enabled() &&
        tailsitter.input_type & Tailsitter::input::TAILSITTER_INPUT_BF_ROLL) {
 ...
 */
float QuadPlopter::get_desired_yaw_rate_cds(bool should_weathervane)
{
    float yaw_cds = 0;
    if (assisted_flight) {
...
float QuadPlopter::get_pilot_desired_climb_rate_cms(void) const
{
    ...
    uint16_t dead_zone = plopter.channel_throttle->get_dead_zone();
    uint16_t trim = (plopter.channel_throttle->get_radio_max() + plopter.channel_throttle->get_radio_min())/2;
    const float throttle_request = plopter.channel_throttle->pwm_to_angle_dz_trim(dead_zone, trim) *0.01f;
 ...
 */
void QuadPlopter::init_throttle_wait(void)
{
    if (plopter.get_throttle_input() >= 10 ||
        plopter.is_flying()) {
        throttle_wait = false;
    } else {
        throttle_wait = true;        
...
// set motor arming
void QuadPlopter::set_armed(bool armed)
{
    if (!initialised) {
        return;
    }
    ...
    if (plopter.control_mode == &plopter.mode_guided) {
 ...
 */
float QuadPlopter::assist_climb_rate_cms(void) const
{
    float climb_rate;
    if (plopter.control_mode->does_auto_throttle()) {
        // use altitude_error_cm, spread over 10s interval
        climb_rate = plopter.altitude_error_cm * 0.1f;
    } else {
        // otherwise estimate from pilot input
        climb_rate = plopter.g.flybywire_climb_rate * (plopter.nav_pitch_cd/(float)plopter.aparm.pitch_limit_max_cd);
        climb_rate *= plopter.get_throttle_input();
    }
 ...
 */
float QuadPlopter::desired_auto_yaw_rate_cds(void) const
{
    float aspeed;
    if (!ahrs.airspeed_estimate(aspeed) || aspeed < plopter.aparm.airspeed_min) {
        aspeed = plopter.aparm.airspeed_min;
    }
    if (aspeed < 1) {
        aspeed = 1;
    }
    float yaw_rate = degrees(GRAVITY_MSS * tanf(radians(plopter.nav_roll_cd*0.01f))/aspeed) * 100;
    return yaw_rate;
}

/*
  return true if the quadplopter should provide stability assistance
 */
bool QuadPlopter::should_assist(float aspeed, bool have_airspeed)
{
    ...
    if (!tailsitter.enabled() && !( (plopter.control_mode->does_auto_throttle() && !plopter.throttle_suppressed)
                                                                      || is_positive(plopter.get_throttle_input())
                                                                      || plopter.is_flying() ) ) {
        // not in a flight mode and condition where it would be safe to turn on vertial lift motors
        // skip this check for tailsitters because the forward and vertial motors are the same and are controled directly by throttle imput unlike other quadplopters
        in_angle_assist = false;
        angle_error_start_ms = 0;
        return false;
    }
    ...
    if (labs(ahrs.roll_sensor) <= plopter.aparm.roll_limit_cd+allowed_envelope_error_cd &&
        ahrs.pitch_sensor < plopter.aparm.pitch_limit_max_cd+allowed_envelope_error_cd &&
        ahrs.pitch_sensor > -(allowed_envelope_error_cd-plopter.aparm.pitch_limit_min_cd)) {
    ...
    if ((labs(ahrs.roll_sensor - plopter.nav_roll_cd) < max_angle_cd &&
         labs(ahrs.pitch_sensor - plopter.nav_pitch_cd) < max_angle_cd)) {
...
/*
  update for transition from quadplopter to fixed wing mode
 */
void SLT_Transition::update()
{
    ...
    float aspeed;
    bool have_airspeed = quadplopter.ahrs.airspeed_estimate(aspeed);

    /*
      see if we should provide some assistance
     */
    if (quadplopter.should_assist(aspeed, have_airspeed)) {
        // the quad should provide some assistance to the plopter
        quadplopter.assisted_flight = true;
        ...
        quadplopter.assisted_flight = false;
    }
    ...
    if (quadplopter.tiltrotor.fully_fwd() && transition_state != TRANSITION_AIRSPEED_WAIT) {
    ...
    if (transition_state < TRANSITION_TIMER) {
        ...
        if (plopter.ahrs.groundspeed() < 3) {
            // until we have some ground speed limit to zero pitch
            plopter.TECS_controller.set_pitch_max_limit(0);
        } else {
            plopter.TECS_controller.set_pitch_max_limit(quadplopter.transition_pitch_max);
        }
    } else if (transition_state < TRANSITION_DONE) {
        plopter.TECS_controller.set_pitch_max_limit((quadplopter.transition_pitch_max+1)*2);
    }
    if (transition_state < TRANSITION_DONE) {
        ...
        plopter.TECS_controller.use_synthetic_airspeed();
    }
    ...
    case TRANSITION_AIRSPEED_WAIT: {
        ...
        if (transition_start_ms != 0 &&
        (quadplopter.transition_failure.timeout > 0) &&
        ((now - transition_start_ms) > ((uint32_t)quadplopter.transition_failure.timeout * 1000))) {
            if (!quadplopter.transition_failure.warned) {
                gcs().send_text(MAV_SEVERITY_CRITICAL, "Transition failed, exceeded time limit");
                quadplopter.transition_failure.warned = true;
            }
            ...
            const bool tiltrotor_with_ground_speed = quadplopter.tiltrotor.enabled() && (plopter.ahrs.groundspeed() > plopter.aparm.airspeed_min * 0.5);
            if ((quadplopter.options & QuadPlopter::OPTION_TRANS_FAIL_TO_FW) && tiltrotor_with_ground_speed) {
                transition_state = TRANSITION_TIMER;
                in_forced_transition = true;
            } else {
                switch (QuadPlopter::TRANS_FAIL::ACTION(quadplopter.transition_failure.action)) {
                    case QuadPlopter::TRANS_FAIL::ACTION::QLAND:
                        plopter.set_mode(plopter.mode_qland, ModeReason::VTOL_FAILED_TRANSITION);
                        break;

                    case QuadPlopter::TRANS_FAIL::ACTION::QRTL:
                        plopter.set_mode(plopter.mode_qrtl, ModeReason::VTOL_FAILED_TRANSITION);
                        quadplopter.poscontrol.set_state(QuadPlopter::QPOS_POSITION1);
                        break;
            ...
            quadplopter.transition_failure.warned = false;
        }

        transition_low_airspeed_ms = now;
        if (have_airspeed && aspeed > plopter.aparm.airspeed_min && !quadplopter.assisted_flight) {
            transition_state = TRANSITION_TIMER;
            airspeed_reached_tilt = quadplopter.tiltrotor.current_tilt;
        ...
        // otherwise the plopter can end up in high-alpha flight with
        // low VTOL thrust and may not complete a transition
        float climb_rate_cms = quadplopter.assist_climb_rate_cms();
        if ((quadplopter.options & QuadPlopter::OPTION_LEVEL_TRANSITION) && !quadplopter.tiltrotor.enabled()) {
            climb_rate_cms = MIN(climb_rate_cms, 0.0f);
        }
        quadplopter.hold_hover(climb_rate_cms);

        if (!quadplopter.tiltrotor.is_vectored()) {
            ...
            // transitions
            quadplopter.attitude_control->reset_yaw_target_and_rate();
            quadplopter.attitude_control->rate_bf_yaw_target(quadplopter.ahrs.get_gyro().z);
        }
        if (quadplopter.tiltrotor.enabled() && !quadplopter.tiltrotor.has_fw_motor()) {
            // tilt rotors without decidated fw motors do not have forward throttle output in this stage
            // prevent throttle I wind up
            plopter.TECS_controller.reset_throttle_I();
        }
        ...
        // multicopter control
        plopter.pitchController.reset_I();
        plopter.rollController.reset_I();

        // give full authority to attitude control
        quadplopter.attitude_control->set_throttle_mix_max(1.0f);
        break;
    }
    ...
    case TRANSITION_TIMER: {
        ...
        if (transition_timer_ms > (unsigned)quadplopter.transition_time_ms) {
            transition_state = TRANSITION_DONE;
        ...
        float trans_time_ms = MAX((float)quadplopter.transition_time_ms.get(),1);
        ...
        quadplopter.attitude_control->set_throttle_mix_value(0.5*transition_scale);
        ...
        if (quadplopter.tiltrotor.enabled() && !quadplopter.tiltrotor.has_vtol_motor() && !quadplopter.tiltrotor.has_fw_motor()) {
            ...
            const float ratio = (constrain_float(quadplopter.tiltrotor.current_tilt, airspeed_reached_tilt, quadplopter.tiltrotor.get_fully_forward_tilt()) - airspeed_reached_tilt) / (quadplopter.tiltrotor.get_fully_forward_tilt() - airspeed_reached_tilt);
        ...
        quadplopter.assisted_flight = true;
        quadplopter.hold_stabilize(throttle_scaled);
    ...
    case TRANSITION_DONE:
        if (!quadplopter.tiltrotor.motors_active()) {
            quadplopter.set_desired_spool_state(AP_Motors::DesiredSpoolState::SHUT_DOWN);
            motors->output();
        }
        set_last_fw_pitch();
        in_forced_transition = false;
...
void SLT_Transition::VTOL_update()
{
    ...
    */
    transition_start_ms = 0;
    transition_low_airspeed_ms = 0;
    if (quadplopter.throttle_wait && !plopter.is_flying()) {
        in_forced_transition = false;
        transition_state = TRANSITION_DONE;
    } else {
        /*
...
/*
  update motor output for quadplopter
 */
void QuadPlopter::update(void)
{
    if (!setup()) {
        return;
    }
    ...
    if (plopter.afs.should_crash_vehicle() && !plopter.afs.terminating_vehicle_via_landing()) {
    ...
    if (!in_vtol_mode() && !in_vtol_airbrake()) {
        ...
        if (plopter.control_mode == &plopter.mode_manual ||
            plopter.control_mode == &plopter.mode_acro ||
            plopter.control_mode == &plopter.mode_training) {
 ...
 */
void QuadPlopter::update_throttle_suppression(void)
{
    ...
    */
    if (!is_zero(plopter.get_throttle_input()) &&
        (rc().arming_check_throttle() ||
         plopter.control_mode->is_vtol_man_throttle() ||
         plopter.channel_throttle->norm_input_dz() > 0)) {
        return;
    }
    ...
    if (plopter.control_mode->does_auto_throttle() && !plopter.throttle_suppressed) {
        return;
    }
    ...
    if (plopter.relative_ground_altitude(plopter.g.rangefinder_landing) > 5) {
        return;
    }

    // allow for takeoff
    if (plopter.control_mode == &plopter.mode_auto && is_vtol_takeoff(plopter.mission.get_current_nav_cmd().id)) {
        return;
    }
...
//  called at 100hz
void QuadPlopter::update_throttle_hover()
{
    if (!available()) {
        return;
    }
    ...
    // do not update if quadplopter forward motor is running (wing may be generating lift)
    // we use the THR_MIN value to account for petrol motors idling at THR_MIN
    if (!tailsitter.enabled() && (SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) > MAX(0,plopter.aparm.throttle_min+10))) {
        return;
    }
    ...
    if (throttle > 0.0f && fabsf(inertial_nav.get_velocity_z_up_cms()) < 60 &&
        ...
        ahrs.airspeed_estimate(aspeed) && aspeed < plopter.aparm.airspeed_min*0.3) {
        // Can we set the time constant automatically
        motors->update_throttle_hover(0.01f);
#if HAL_GYROFFT_ENABLED
        plopter.gyro_fft.update_freq_hover(0.01f, motors->get_throttle_out());
#endif
    }
}
/*
 ...
 */
void QuadPlopter::motors_output(bool run_rate_controller)
{
    ...
    if ((options & OPTION_DISARMED_TILT) || (options & OPTION_DELAY_ARMING)) {
        if (plopter.arming.get_delay_arming()) {
...
#if ADVANCED_FAILSAFE == ENABLED
    if (!hal.util->get_soft_armed() ||
        (plopter.afs.should_crash_vehicle() && !plopter.afs.terminating_vehicle_via_landing()) ||
    ...
    if (esc_calibration && AP_Notify::flags.esc_calibration && plopter.control_mode == &plopter.mode_qstabilize) {
 ...
 */
bool QuadPlopter::handle_do_vtol_transition(enum MAV_VTOL_STATE state) const
{
    if (!available()) {
    ...
    case MAV_VTOL_STATE_MC:
        if (!plopter.auto_state.vtol_mode) {
 ...
 */
bool QuadPlopter::in_vtol_auto(void) const
{
    if (!available()) {
        return false;
    }
    ...
    uint16_t id = plopter.mission.get_current_nav_cmd().id;
    switch (id) {
    ...
    case MAV_CMD_NAV_LOITER_TO_ALT:
        return plopter.auto_state.vtol_loiter;
 ...
 */
bool QuadPlopter::in_vtol_mode(void) const
{
    if (!available()) {
        return false;
    }
    ...
    if (plopter.control_mode->is_vtol_mode()) {
        return true;
    }
    if (plopter.control_mode->is_guided_mode()
        && plopter.auto_state.vtol_loiter &&
    ...
    if (in_vtol_auto()) {
        if (!plopter.auto_state.vtol_loiter || poscontrol.get_state() > QPOS_AIRBRAKE) {
            return true;
        }
    }
    return false;
 ...
 */
bool QuadPlopter::in_vtol_posvel_mode(void) const
{
    if (!available()) {
        return false;
    }
    return (plopter.control_mode == &plopter.mode_qloiter ||
            plopter.control_mode == &plopter.mode_qland ||
            plopter.control_mode == &plopter.mode_qrtl ||
#if QAUTOTUNE_ENABLED
            plopter.control_mode == &plopter.mode_qautotune ||
#endif
            (plopter.control_mode->is_guided_mode() &&
            plopter.auto_state.vtol_loiter &&
             poscontrol.get_state() > QPOS_APPROACH) ||
 ...
 */
void QuadPlopter::update_land_positioning(void)
{
    ...
    float roll_in = plopter.channel_roll->get_control_in() * scale;
    float pitch_in = plopter.channel_pitch->get_control_in() * scale;
    ...
    const float dt = plopter.scheduler.get_loop_period_s();
 ...
 */
void QuadPlopter::run_xy_controller(float accel_limit)
{
 ...
 */
void QuadPlopter::poscontrol_init_approach(void)
{
    const float dist = plopter.current_loc.get_distance(plopter.next_WP_loc);
        ...
        if (dist < transition_threshold()) {
            if (tailsitter.enabled() || motors->get_desired_spool_state() == AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED) {
                ...
                gcs().send_text(MAV_SEVERITY_INFO,"VTOL airbrake v=%.1f d=%.0f sd=%.0f h=%.1f",
                                plopter.ahrs.groundspeed(),
                                dist,
                                stopping_distance(),
                                plopter.relative_ground_altitude(plopter.g.rangefinder_landing));
 ...
 */
void QuadPlopter::PosControlState::set_state(enum position_control_state s)
{
    const uint32_t now = AP_HAL::millis();
    if (state != s) {
        auto &qp = plopter.quadplopter;
        ...
        if (s == QPOS_POSITION1) {
            ...
            pos1_speed_limit = plopter.ahrs.groundspeed_vector().length();
            ...
            Vector2f rpos;
            last_pos_reset_ms = plopter.ahrs.getLastPosNorthEastReset(rpos);
 ...
 */
void QuadPlopter::vtol_position_controller(void)
{
    if (!setup()) {
        return;
    }

    const Location &loc = plopter.next_WP_loc;
    ...
    case QPOS_AIRBRAKE: {
        ...
        const float groundspeed = plopter.ahrs.groundspeed();
        const float distance = plopter.auto_state.wp_distance;
        const float closing_speed = closing_vel.length();
        const float desired_closing_speed = desired_closing_vel.length();
        if (!plopter.ahrs.airspeed_estimate(aspeed)) {
            aspeed = groundspeed;
        }
        ...
        const float aspeed_threshold = MAX(plopter.aparm.airspeed_min-2, assist_speed);

        // run fixed wing navigation
        plopter.nav_controller->update_waypoint(plopter.current_loc, loc);

        // use TECS for throttle
        SRV_Channels::set_output_scaled(SRV_Channel::k_throttle, plopter.TECS_controller.get_throttle_demand());

        // use TECS for pitch
        int32_t commanded_pitch = plopter.TECS_controller.get_pitch_demand();
        plopter.nav_pitch_cd = constrain_int32(commanded_pitch, plopter.pitch_limit_min_cd, plopter.aparm.pitch_limit_max_cd.get());
        if (poscontrol.get_state() == QPOS_AIRBRAKE) {
            // don't allow down pitch in airbrake
            plopter.nav_pitch_cd = MAX(plopter.nav_pitch_cd, 0);
        }
        ...
        if (poscontrol.get_state() == QPOS_AIRBRAKE &&
            ...
            (aspeed < aspeed_threshold ||
             ...
             labs(plopter.ahrs.roll_sensor - plopter.nav_roll_cd) > attitude_error_threshold_cd ||
             labs(plopter.ahrs.pitch_sensor - plopter.nav_pitch_cd) > attitude_error_threshold_cd)) {
            gcs().send_text(MAV_SEVERITY_INFO,"VTOL position1 v=%.1f d=%.1f h=%.1f dc=%.1f",
                            (double)groundspeed,
                            (double)plopter.auto_state.wp_distance,
                            plopter.relative_ground_altitude(plopter.g.rangefinder_landing),
        ...
        if (!tiltrotor.enabled() && !tailsitter.enabled()) {
            ...
            */
            bool throttle_saturated = SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) >= plopter.aparm.throttle_max;
            if (throttle_saturated &&
                motors->get_desired_spool_state() < AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED &&
                plopter.auto_state.sink_rate > 0.2 && aspeed < aspeed_threshold+4) {
    ...
    case QPOS_POSITION1: {
        ...
        const Vector2f diff_wp = plopter.current_loc.get_distance_NE(loc);
        ...
        plopter.nav_roll_cd = pos_control->get_roll_cd();
        plopter.nav_pitch_cd = pos_control->get_pitch_cd();

        if (transition->set_VTOL_roll_pitch_limit(plopter.nav_roll_cd, plopter.nav_pitch_cd)) {
        ...
        if ((plopter.auto_state.wp_distance < position2_dist_threshold) && tiltrotor.tilt_angle_achieved()) {
            ...
            gcs().send_text(MAV_SEVERITY_INFO,"VTOL position2 started v=%.1f d=%.1f h=%.1f",
                            (double)ahrs.groundspeed(), (double)plopter.auto_state.wp_distance,
                            plopter.relative_ground_altitude(plopter.g.rangefinder_landing));
        }
        break;
    }
    ...
    case QPOS_LAND_FINAL:
        ...
        if (should_relax()) {
            ...
            Vector2f vel_cms = poscontrol.target_vel_cms.xy() + landing_velocity*100;
            Vector2f rpos;
            const uint32_t last_reset_ms = plopter.ahrs.getLastPosNorthEastReset(rpos);
    ...
    case QPOS_POSITION2: {
        ...
        if (plopter.control_mode == &plopter.mode_auto) {
            switch (plopter.mission.get_current_nav_cmd().id) {
            case MAV_CMD_NAV_LOITER_UNLIM:
            case MAV_CMD_NAV_LOITER_TIME:
            case MAV_CMD_NAV_LOITER_TURNS:
            case MAV_CMD_NAV_LOITER_TO_ALT:
        ...
        if (plopter.control_mode == &plopter.mode_guided || vtol_loiter_auto) {
            plopter.ahrs.get_location(plopter.current_loc);
            int32_t target_altitude_cm;
            if (!plopter.next_WP_loc.get_alt_cm(Location::AltFrame::ABOVE_HOME,target_altitude_cm)) {
                break;
            }
            if (poscontrol.slow_descent &&
                plopter.prev_WP_loc.get_distance(plopter.next_WP_loc) > 50) {
                // gradually descend as we approach target
                plopter.auto_state.wp_proportion = plopter.current_loc.line_path_proportion(plopter.prev_WP_loc, plopter.next_WP_loc);
                int32_t prev_alt;
                if (plopter.prev_WP_loc.get_alt_cm(Location::AltFrame::ABOVE_HOME,prev_alt)) {
                    target_altitude_cm = linear_interpolate(prev_alt,
                                                         target_altitude_cm,
                                                         plopter.auto_state.wp_proportion,
                                                         0, 1);
                }
            }
#if AP_TERRAIN_AVAILABLE
            float terrain_altitude_offset;
            if (plopter.next_WP_loc.terrain_alt && plopter.terrain.height_terrain_difference_home(terrain_altitude_offset, true)) {
        ...
        } else if (plopter.control_mode == &plopter.mode_qrtl) {
            Location loc2 = loc;
...
/*
  ...
  off pointing at the destination. quadplopters are often
  unstable when flying sideways or backwards
*/
float QuadPlopter::get_scaled_wp_speed(float target_bearing_deg) const
{
    const float yaw_difference = fabsf(wrap_180(degrees(plopter.ahrs.yaw) - target_bearing_deg));
...
/*
  setup the target position based on plopter.next_WP_loc
 */
void QuadPlopter::setup_target_position(void)
{
    const Location &loc = plopter.next_WP_loc;
    Location origin;
    ...
    poscontrol.target_cm.z = plopter.next_WP_loc.alt - origin.alt;
 ...
 */
void QuadPlopter::takeoff_controller(void)
{
    if (!hal.util->get_soft_armed()) {
        return;
    }
    ...
    */
    ...
    if (takeoff_navalt_min > 0) {
        ...
        const float alt = plopter.current_loc.alt*0.01;
    ...
    if (plopter.control_mode == &plopter.mode_guided && guided_takeoff) {
        ...
        if (ahrs.get_origin(origin)) {
            ...
            float pos_z = margin_cm + plopter.next_WP_loc.alt - origin.alt;
            vel_z = 0;
 ...
 */
void QuadPlopter::waypoint_controller(void)
{
    setup_target_position();
    ...
    if (!loc.same_latlon_as(last_auto_target) ||
        plopter.next_WP_loc.alt != last_auto_target.alt ||
    ...
    */
    ...
    plopter.nav_roll_cd = wp_nav->get_roll();
    plopter.nav_pitch_cd = wp_nav->get_pitch();
 ...
 */
void QuadPlopter::control_auto(void)
{
    if (!setup()) {
        return;
    }
 ...
 */
bool QuadPlopter::do_vtol_takeoff(const AP_Mission::Mission_Command& cmd)
{
    if (!setup()) {
        return false;
    }
    ...
    loc.lat = 0;
    loc.lng = 0;
    plopter.set_next_WP(loc);
    if (options & OPTION_RESPECT_TAKEOFF_FRAME) {
        if (plopter.current_loc.alt >= plopter.next_WP_loc.alt) {
            // we are above the takeoff already, no need to do anything
            return false;
        }
    } else {
        plopter.next_WP_loc.alt = plopter.current_loc.alt + cmd.content.location.alt;
    }
    throttle_wait = false;
    ...
    plopter.nav_controller->update_waypoint(plopter.current_loc, plopter.next_WP_loc);
    ...
    const float d_total = (plopter.next_WP_loc.alt - plopter.current_loc.alt) * 0.01f;
 ...
 */
bool QuadPlopter::do_vtol_land(const AP_Mission::Mission_Command& cmd)
{
    if (!setup()) {
        return false;
    }

    plopter.set_next_WP(cmd.content.location);
 ...
 */
bool QuadPlopter::verify_vtol_takeoff(const AP_Mission::Mission_Command &cmd)
{
    if (!available()) {
        return true;
    }
    ...
    if (is_positive(takeoff_failure_scalar) && ((now - takeoff_start_time_ms) > takeoff_time_limit_ms)) {
        ...
        plopter.set_mode(plopter.mode_qland, ModeReason::VTOL_FAILED_TAKEOFF);
        return false;
    }

#if AP_AIRSPEED_ENABLED
    if (is_positive(maximum_takeoff_airspeed) && (plopter.airspeed.get_airspeed() > maximum_takeoff_airspeed)) {
    ...
    transition->restart();
    plopter.TECS_controller.set_pitch_max_limit(transition_pitch_max);
 ...
 */
bool QuadPlopter::land_detector(uint32_t timeout_ms)
{
 ...
 */
bool QuadPlopter::check_land_complete(void)
{
    if (poscontrol.get_state() != QPOS_LAND_FINAL) {
    ...
    if (land_detector(4000)) {
        ...
        if (plopter.control_mode != &plopter.mode_auto ||
            !plopter.mission.continue_after_land()) {
            // disarm on land unless we have MIS_OPTIONS setup to
            // continue after land in AUTO
            plopter.arming.disarm(AP_Arming::Method::LANDED);
        }
        return true;
    }
    return false;
 ...
 */
bool QuadPlopter::check_land_final(void)
{
    float height_above_ground = plopter.relative_ground_altitude(plopter.g.rangefinder_landing);
 ...
 */
bool QuadPlopter::verify_vtol_land(void)
{
    if (!available()) {
        return true;
    }
    ...
    if (poscontrol.get_state() == QPOS_POSITION2) {
        ...
        Vector3f vel_ned;
        UNUSED_RESULT(plopter.ahrs.get_velocity_NED(vel_ned));
        ...
        if (reached_position &&
...
#endif
            last_land_final_agl = plopter.relative_ground_altitude(plopter.g.rangefinder_landing);
            ...
            if (plopter.control_mode == &plopter.mode_auto) {
                ...
                // available
                plopter.set_next_WP(plopter.mission.get_current_nav_cmd().content.location);
            } else {
                plopter.set_next_WP(plopter.next_WP_loc);
                plopter.next_WP_loc.alt = ahrs.get_home().alt;
            }
        }
    }
    ...
    if (check_land_complete() && plopter.mission.continue_after_land()) {
...
void QuadPlopter::Log_Write_QControl_Tuning()
{
    float des_alt_m = 0.0f;
    int16_t target_climb_rate_cms = 0;
    if (plopter.control_mode != &plopter.mode_qstabilize) {
    ...
    struct log_QControl_Tuning pkt = {
        ...
        baro_alt            : int32_t(plopter.barometer.get_altitude() * 100),
    ...
    plopter.logger.WriteBlock(&pkt, sizeof(pkt));
...
/*
  ...
  lift motors.
 */
float QuadPlopter::forward_throttle_pct()
{
    /*
      ...
      recovery modes for a quadplopter and need to be as simple as
      possible. They will drift with the wind.
    */
    if (plopter.control_mode == &plopter.mode_qacro ||
        plopter.control_mode == &plopter.mode_qstabilize ||
        plopter.control_mode == &plopter.mode_qhover) {
    ...
    */
    ...
    Vector3f vel_ned;
    if (!plopter.ahrs.get_velocity_NED(vel_ned)) {
    ...
    fwd_vel_error /= MAX(plopter.aparm.airspeed_max, 5);
    ...
    // WP nav speed.
    fwd_vel_error -= (wp_nav->get_default_speed_xy() * 0.01f) * plopter.nav_pitch_cd / (float)plopter.aparm.pitch_limit_max_cd;
    ...
    int8_t fwd_throttle_min = plopter.have_reverse_thrust() ? 0 : plopter.aparm.throttle_min;
    vel_forward.integrator = constrain_float(vel_forward.integrator, fwd_throttle_min, plopter.aparm.throttle_cruise);
    ...
    if (in_vtol_land_approach()) {
        ...
        vel_forward.last_pct = vel_forward.integrator;
              ...
              (plopter.g.rangefinder_landing && (plopter.rangefinder.status_orient(ROTATION_PITCH_270) == RangeFinder::Status::OutOfRangeLow))) {
 ...
 */
float QuadPlopter::get_weathervane_yaw_rate_cds(void)
{
    /*
    ...
    */
    ...
    const bool is_takeoff = in_vtol_auto() && is_vtol_takeoff(plopter.mission.get_current_nav_cmd().id);
    float wv_output;
    if (weathervane->get_yaw_out(wv_output,
                                     plopter.channel_rudder->get_control_in(),
                                     plopter.relative_ground_altitude(plopter.g.rangefinder_landing),
 ...
 */
void QuadPlopter::guided_start(void)
{
 ...
 */
void QuadPlopter::guided_update(void)
{
    if (plopter.control_mode == &plopter.mode_guided && guided_takeoff && plopter.current_loc.alt < plopter.next_WP_loc.alt) {
        throttle_wait = false;
        set_desired_spool_state(AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED);
        takeoff_controller();
    } else {
...
void QuadPlopter::afs_terminate(void)
{
    if (available()) {
        set_desired_spool_state(AP_Motors::DesiredSpoolState::SHUT_DOWN);
        motors->output();
 ...
 */
bool QuadPlopter::guided_mode_enabled(void)
{
    if (!available()) {
        return false;
    }
    // only use quadplopter guided when in AUTO or GUIDED mode
    if (plopter.control_mode != &plopter.mode_guided && plopter.control_mode != &plopter.mode_auto) {
        return false;
    }
    if (plopter.control_mode == &plopter.mode_auto &&
        plopter.mission.get_current_nav_cmd().id == MAV_CMD_NAV_LOITER_TURNS) {
 ...
 */
void QuadPlopter::set_alt_target_current(void)
{
    pos_control->set_pos_target_z_cm(inertial_nav.get_position_z_up_cm());
}

// user initiated takeoff for guided mode
bool QuadPlopter::do_user_takeoff(float takeoff_altitude)
{
    if (plopter.control_mode != &plopter.mode_guided) {
    ...
    plopter.auto_state.vtol_loiter = true;
    plopter.prev_WP_loc = plopter.current_loc;
    plopter.next_WP_loc = plopter.current_loc;
    plopter.next_WP_loc.alt += takeoff_altitude*100;
    set_desired_spool_state(AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED);
    guided_start();
    guided_takeoff = true;
...
bool QuadPlopter::using_wp_nav(void) const
{
    if (plopter.control_mode == &plopter.mode_qloiter || plopter.control_mode == &plopter.mode_qland) {
        return true;
    }
    return false;
}
 ...
 */
MAV_TYPE QuadPlopter::get_mav_type(void) const
{
    if (mav_type.get() == 0) {
...
*/
bool QuadPlopter::is_vtol_takeoff(uint16_t id) const
{
...
*/
bool QuadPlopter::is_vtol_land(uint16_t id) const
{
    if (id == MAV_CMD_NAV_VTOL_LAND) {
        if (options & QuadPlopter::OPTION_MISSION_LAND_FW_APPROACH) {
            return plopter.vtol_approach_s.approach_stage == Plopter::Landing_ApproachStage::VTOL_LANDING;
        } else {
            return true;
        }
    }
 ...
 */
bool QuadPlopter::in_transition(void) const
{
    return available() && transition->active();
}

/*
  calculate current stopping distance for a quadplopter in fixed wing flight
 */
float QuadPlopter::stopping_distance(float ground_speed_squared) const
{
 ...
 */
float QuadPlopter::accel_needed(float stop_distance, float ground_speed_squared) const
{
...
/*
  calculate current stopping distance for a quadplopter in fixed wing flight
 */
float QuadPlopter::stopping_distance(void)
{
    return stopping_distance(plopter.ahrs.groundspeed_vector().length_squared());
}
 ...
 */
float QuadPlopter::transition_threshold(void)
{
    // 1.5 times stopping distance for cruise speed
    return 1.5 * stopping_distance(sq(plopter.aparm.airspeed_cruise_cm*0.01));
}
...
void QuadPlopter::update_throttle_mix(void)
{
    // update filtered acceleration
    Vector3f accel_ef = ahrs.get_accel_ef_blended();
    accel_ef.z += GRAVITY_MSS;
    throttle_mix_accel_ef_filter.apply(accel_ef, plopter.scheduler.get_loop_period_s());
    ...
    if (plopter.control_mode->is_vtol_man_throttle()) {
        // manual throttle
        if (!is_positive(plopter.get_throttle_input()) && !air_mode_active()) {
            attitude_control->set_throttle_mix_min();
        } else {
 ...
 */
bool QuadPlopter::in_vtol_land_approach(void) const
{
    if (plopter.control_mode == &plopter.mode_qrtl &&
        poscontrol.get_state() <= QPOS_POSITION2) {
        return true;
    }
    if (in_vtol_auto()) {
        if (is_vtol_land(plopter.mission.get_current_nav_cmd().id) &&
            (poscontrol.get_state() == QPOS_APPROACH ||
 ...
 */
bool QuadPlopter::in_vtol_land_descent(void) const
{
    if (plopter.control_mode == &plopter.mode_qrtl &&
        (poscontrol.get_state() == QPOS_LAND_DESCEND || poscontrol.get_state() == QPOS_LAND_FINAL)) {
        return true;
    }
    if (in_vtol_auto() && is_vtol_land(plopter.mission.get_current_nav_cmd().id) &&
        (poscontrol.get_state() == QPOS_LAND_DESCEND || poscontrol.get_state() == QPOS_LAND_FINAL)) {
        return true;
    }
    return false;
 ...
 */
bool QuadPlopter::in_vtol_land_final(void) const
{
 ...
 */
bool QuadPlopter::in_vtol_land_sequence(void) const
{
    return plopter.control_mode == &plopter.mode_qrtl || in_vtol_land_approach() || in_vtol_land_descent() || in_vtol_land_final();
}

/*
  see if we are in the VTOL position control phase of a landing
 */
bool QuadPlopter::in_vtol_land_poscontrol(void) const
{
    if (in_vtol_auto() && is_vtol_land(plopter.mission.get_current_nav_cmd().id) &&
        poscontrol.get_state() >= QPOS_POSITION1) {
        return true;
 ...
 */
bool QuadPlopter::in_vtol_airbrake(void) const
{
    if (plopter.control_mode == &plopter.mode_qrtl &&
        poscontrol.get_state() == QPOS_AIRBRAKE) {
        return true;
    ...
    if (plopter.control_mode == &plopter.mode_auto &&
        is_vtol_land(plopter.mission.get_current_nav_cmd().id) &&
        poscontrol.get_state() == QPOS_AIRBRAKE) {
        return true;
    }
    return false;
...
bool QuadPlopter::show_vtol_view() const
{
 ...
 */
bool QuadPlopter::use_fw_attitude_controllers(void) const
{
    if (available() &&
        motors->armed() &&
        motors->get_desired_spool_state() >= AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED &&
...
*/
Vector2f QuadPlopter::landing_closing_velocity()
{
    Vector2f landing_velocity;
    if (AP_HAL::millis() - poscontrol.last_velocity_match_ms < 1000) {
        landing_velocity = poscontrol.velocity_match;
...
*/
Vector2f QuadPlopter::landing_desired_closing_velocity()
{
    if (poscontrol.get_state() >= QPOS_LAND_DESCEND) {
        return Vector2f(0,0);
    }
    const Vector2f diff_wp = plopter.current_loc.get_distance_NE(plopter.next_WP_loc);
...
*/
float QuadPlopter::get_land_airspeed(void)
{
    if (poscontrol.get_state() == QPOS_APPROACH ||
        plopter.control_mode == &plopter.mode_rtl) {
        float land_airspeed = plopter.TECS_controller.get_land_airspeed();
        if (!is_positive(land_airspeed)) {
            land_airspeed = plopter.aparm.airspeed_cruise_cm * 0.01;
        }
        float cruise_airspeed = plopter.aparm.airspeed_cruise_cm * 0.01;
        float time_to_landing = plopter.auto_state.wp_distance / MAX(land_airspeed, 5);
        /*
    ...
    const float eas2tas = plopter.ahrs.get_EAS2TAS();
    const Vector3f wind = plopter.ahrs.wind_estimate();
    vel.x -= wind.x;
    vel.y -= wind.y;
    vel /= eas2tas;
    return vel.length();
...
void QuadPlopter::set_desired_spool_state(AP_Motors::DesiredSpoolState state)
{
...
bool QuadPlopter::air_mode_active() const
{
 ...
 */
float QuadPlopter::FW_vector_throttle_scaling()
{
...
QuadPlopter *QuadPlopter::_singleton = nullptr;

bool SLT_Transition::set_FW_roll_limit(int32_t& roll_limit_cd)
{
    if (quadplopter.assisted_flight && (transition_state == TRANSITION_AIRSPEED_WAIT || transition_state == TRANSITION_TIMER) &&
        (quadplopter.options & QuadPlopter::OPTION_LEVEL_TRANSITION)) {
        // the user wants transitions to be kept level to within LEVEL_ROLL_LIMIT
        roll_limit_cd = MIN(roll_limit_cd, plopter.g.level_roll_limit*100);
        return true;
    }
    return false;
}
...
bool SLT_Transition::allow_update_throttle_mix() const
{
    ...
    return !(quadplopter.assisted_flight && (transition_state == TRANSITION_AIRSPEED_WAIT || transition_state == TRANSITION_TIMER));
}
...
{
    bool ret = false;
    const int16_t angle_max = quadplopter.aparm.angle_max;
    ...
    float airspeed;
    if (pitch_cd > angle_max &&
        plopter.ahrs.airspeed_estimate(airspeed) && airspeed < 0.5 * plopter.aparm.airspeed_min) {
        const float max_limit_cd = linear_interpolate(angle_max, 4500,
                                                      airspeed,
                                                      0, 0.5 * plopter.aparm.airspeed_min);
    ...
    if (quadplopter.back_trans_pitch_limit_ms <= 0) {
        // time based pitch envelope disabled
        return ret;
    }

    const uint32_t limit_time_ms = quadplopter.back_trans_pitch_limit_ms;
    ...
    const float max_limit_cd = linear_interpolate(MAX(last_fw_nav_pitch_cd,0), MIN(angle_max,plopter.aparm.pitch_limit_max_cd),
                                            dt,
                                            0, limit_time_ms);
    ...
    /*
        ...
        to landing
    */
    const float min_limit_cd = linear_interpolate(MIN(last_fw_nav_pitch_cd,0), MAX(-angle_max,plopter.aparm.pitch_limit_min_cd),
                                                  dt,
                                                  0, limit_time_ms);

    if (plopter.nav_pitch_cd < min_limit_cd) {
        plopter.nav_pitch_cd = min_limit_cd;
        return true;
    }
...
void SLT_Transition::set_last_fw_pitch()
{
    ...
    last_fw_nav_pitch_cd = plopter.nav_pitch_cd;
}
...
{
    if (quadplopter.in_vtol_mode()) {
        QuadPlopter::position_control_state state = quadplopter.poscontrol.get_state();
        if ((state == QuadPlopter::position_control_state::QPOS_AIRBRAKE) || (state == QuadPlopter::position_control_state::QPOS_POSITION1)) {
 ...
 */
bool QuadPlopter::in_vtol_takeoff(void) const
{
    if (in_vtol_auto() && is_vtol_takeoff(plopter.mission.get_current_nav_cmd().id)) {
        return true;
    }
    return false;
}
...
void QuadPlopter::mode_enter(void)
{
    if (available()) {
        pos_control->set_lean_angle_max_cd(0);
    }
