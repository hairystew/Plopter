...
// ---------------------------------------------
void Plopter::set_auto_armed(bool b)
{
 ...
 */
void Plopter::set_simple_mode(SimpleMode b)
{
    if (simple_mode != b) {
        switch (b) {
...
// ---------------------------------------------
void Plopter::set_failsafe_radio(bool b)
{
    // only act on changes
    // -------------------
    if(failsafe.radio != b) {
...
// ---------------------------------------------
void Plopter::set_failsafe_gcs(bool b)
{
    failsafe.gcs = b;
...
void Plopter::update_using_interlock()
{
#if FRAME_CONFIG == HELI_FRAME
    // heliplopters are always using motor interlock
    ap.using_interlock = true;
#else
    // check if we are using motor interlock control on an aux switch or are in throw mode
    // which uses the interlock to stop motors while the plopter is being thrown
