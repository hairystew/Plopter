...
bool ModeThermal::_enter()
{
    ...
    plopter.g2.soaring_controller.init_thermalling();
    plopter.g2.soaring_controller.get_target(plopter.next_WP_loc); // ahead on flight path
...
void ModeThermal::update_soaring()
{
    ...
    plopter.g2.soaring_controller.update_thermalling();  // Update estimate
    ...
    if (plopter.g2.soaring_controller.max_radius >= 0 &&
        sq(position.x)+sq(position.y) > sq(plopter.g2.soaring_controller.max_radius) &&
        plopter.previous_mode->mode_number()!=Mode::Number::AUTO) {
        ...
        plopter.set_mode(plopter.mode_rtl, ModeReason::SOARING_DRIFT_EXCEEDED);
        return;
    }
    ...
    if (plopter.previous_mode == &plopter.mode_auto) {
        AP_Mission::Mission_Command current_nav_cmd = plopter.mission.get_current_nav_cmd();
        AP_Mission::Mission_Command prev_nav_cmd;

        if (!(plopter.mission.get_next_nav_cmd(plopter.mission.get_prev_nav_cmd_with_wp_index(), prev_nav_cmd) &&
    ...
    const SoaringController::LoiterStatus loiterStatus = plopter.g2.soaring_controller.check_cruise_criteria(prev_wp/100, next_wp/100);
    ...
    if (loiterStatus == SoaringController::LoiterStatus::GOOD_TO_KEEP_LOITERING) {
        ...
        plopter.loiter.sum_cd = 0;
        plopter.soaring_mode_timer_ms = AP_HAL::millis();

        //update the wp location
        plopter.g2.soaring_controller.get_target(plopter.next_WP_loc);

        return;
    }

    // Some other loiter status, we need to think about exiting loiter.
    const uint32_t time_in_loiter_ms = AP_HAL::millis() - plopter.soaring_mode_timer_ms;
    const uint32_t timeout = MIN(1000*plopter.g2.soaring_controller.get_circling_time(), 20000);
...
void ModeThermal::navigate()
{
    ...
    const float radius = plopter.g2.soaring_controller.get_thermalling_radius();
...
bool ModeThermal::exit_heading_aligned() const
{
    ...
    switch (plopter.previous_mode->mode_number()) {
    case Mode::Number::AUTO: {
        //Get the lat/lon of next Nav waypoint after this one:
        AP_Mission::Mission_Command current_nav_cmd = plopter.mission.get_current_nav_cmd();
        return plopter.mode_loiter.isHeadingLinedUp(plopter.next_WP_loc, current_nav_cmd.content.location);
    }
    case Mode::Number::FLY_BY_WIRE_B:
        return (!AP::ahrs().home_is_set() || plopter.mode_loiter.isHeadingLinedUp(plopter.next_WP_loc, AP::ahrs().get_home()));
    case Mode::Number::CRUISE:
        int32_t target_heading_cd;
        return (!plopter.mode_cruise.get_target_heading_cd(target_heading_cd) || plopter.mode_loiter.isHeadingLinedUp_cd(target_heading_cd));
    default:
        break;
    }
    return true;
...
{
    gcs().send_text(MAV_SEVERITY_INFO, "Soaring: %s, restoring %s", reason, plopter.previous_mode->name());
    plopter.set_mode(*plopter.previous_mode, modereason);
}
