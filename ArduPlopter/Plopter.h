...
/*
  main APM:Plopter class
 */
class Plopter : public AP_Vehicle {
public:
    friend class GCS_MAVLINK_Plopter;
    friend class Parameters;
    friend class ParametersG2;
    friend class AP_Arming_Plopter;
    friend class QuadPlopter;
    friend class QAutoTune;
    friend class AP_Tuning_Plopter;
    friend class AP_AdvancedFailsafe_Plopter;
    friend class AP_Avoidance_Plopter;
    friend class GCS_Plopter;
    friend class RC_Channel_Plopter;
    friend class RC_Channels_Plopter;
...
private:
    ...
    */
    ...
    // GCS selection
    GCS_Plopter _gcs; // avoid using this; use gcs()
    GCS_Plopter &gcs() { return _gcs; }
    ...
    // Mission library
    AP_Mission mission{
            FUNCTOR_BIND_MEMBER(&Plopter::start_command_callback, bool, const AP_Mission::Mission_Command &),
            FUNCTOR_BIND_MEMBER(&Plopter::verify_command_callback, bool, const AP_Mission::Mission_Command &),
            FUNCTOR_BIND_MEMBER(&Plopter::exit_mission_callback, void)};
    ...
    AP_Landing landing{mission,ahrs,&TECS_controller,nav_controller,aparm,
            FUNCTOR_BIND_MEMBER(&Plopter::set_target_altitude_proportion, void, const Location&, float),
            FUNCTOR_BIND_MEMBER(&Plopter::constrain_target_altitude_location, void, const Location&, const Location&),
            FUNCTOR_BIND_MEMBER(&Plopter::adjusted_altitude_cm, int32_t),
            FUNCTOR_BIND_MEMBER(&Plopter::adjusted_relative_altitude_cm, int32_t),
            FUNCTOR_BIND_MEMBER(&Plopter::disarm_if_autoland_complete, void),
            FUNCTOR_BIND_MEMBER(&Plopter::update_flight_stage, void)};
#if HAL_ADSB_ENABLED
    AP_ADSB adsb;

    // avoidance of adsb enabled vehicles (normally manned vehicles)
    AP_Avoidance_Plopter avoidance_adsb{adsb};
#endif
    ...
    */
    ...
    // The plopter's current location
...
#if HAL_QUADPLANE_ENABLED
    // support for quadcopter-plopter
    QuadPlopter quadplopter{ahrs};
#endif

    // support for transmitter tuning
    AP_Tuning_Plopter tuning;
