...
{
    if (!plopter.optflow.enabled() || !plopter.optflow.healthy()) {
        return false;
    }
    ...
    if (!plopter.pos_control->is_active_z()) {
        pos_control->init_z_controller();
    }

    flow_filter.set_cutoff_frequency(plopter.scheduler.get_loop_rate_hz(), flow_filter_hz.get());

    quality_filtered = 0;
    flow_pi_xy.reset_I();
    limited = false;

    flow_pi_xy.set_dt(1.0/plopter.scheduler.get_loop_rate_hz());

    // start with INS height
    last_ins_height = plopter.inertial_nav.get_position_z_up_cm() * 0.01;
    height_offset = 0;
...
void ModeFlowHold::flowhold_flow_to_angle(Vector2f &bf_angles, bool stick_input)
{
    ...
    float ins_height = plopter.inertial_nav.get_position_z_up_cm() * 0.01;
    ...
    ef_output += xy_I;
    ef_output *= plopter.aparm.angle_max;

    // convert to body frame
    bf_angles += plopter.ahrs.earth_to_body2D(ef_output);

    // set limited flag to prevent integrator windup
    limited = fabsf(bf_angles.x) > plopter.aparm.angle_max || fabsf(bf_angles.y) > plopter.aparm.angle_max;

    // constrain to angle limit
    bf_angles.x = constrain_float(bf_angles.x, -plopter.aparm.angle_max, plopter.aparm.angle_max);
    bf_angles.y = constrain_float(bf_angles.y, -plopter.aparm.angle_max, plopter.aparm.angle_max);

// @LoggerMessage: FHLD
// @Description: FlowHold mode messages
// @URL: https://ardupilot.org/plopter/docs/flowhold-mode.html
...
void ModeFlowHold::run()
{
    ...
    target_climb_rate = constrain_float(target_climb_rate, -get_pilot_speed_dn(), plopter.g.pilot_speed_up);

    // get pilot's desired yaw rate
    float target_yaw_rate = get_pilot_desired_yaw_rate(plopter.channel_yaw->norm_input_dz());

    // Flow Hold State Machine Determination
    AltHoldModeState flowhold_state = get_alt_hold_state(target_climb_rate);

    if (plopter.optflow.healthy()) {
        const float filter_constant = 0.95;
        quality_filtered = filter_constant * quality_filtered + (1-filter_constant) * plopter.optflow.quality();
    } else {
        quality_filtered = 0;
    }
    ...
    case AltHold_MotorStopped:
        plopter.motors->set_desired_spool_state(AP_Motors::DesiredSpoolState::SHUT_DOWN);
        plopter.attitude_control->reset_rate_controller_I_terms();
        plopter.attitude_control->reset_yaw_target_and_rate();
        plopter.pos_control->relax_z_controller(0.0f);   // forces throttle output to decay to zero
        flow_pi_xy.reset_I();
        break;
    ...
    int16_t pitch_in = plopter.channel_pitch->get_control_in();
    float angle_max = plopter.aparm.angle_max;
    get_pilot_desired_lean_angles(bf_angles.x, bf_angles.y, angle_max, attitude_control->get_althold_lean_angle_max_cd());

    if (quality_filtered >= flow_min_quality &&
        AP_HAL::millis() - plopter.arm_time_ms > 3000) {
    ...
    // apply avoidance
    plopter.avoid.adjust_roll_pitch(bf_angles.x, bf_angles.y, plopter.aparm.angle_max);
#endif

    // call attitude controller
    plopter.attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(bf_angles.x, bf_angles.y, target_yaw_rate);
...
void ModeFlowHold::update_height_estimate(void)
{
    ...
    if (!hal.util->get_soft_armed() ||
        plopter.motors->get_desired_spool_state() != AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED ||
        AP_HAL::millis() - plopter.arm_time_ms < 1500) {
    ...
    Vector3f delta_vel;
    float delta_vel_dt;
    if (!plopter.ins.get_delta_velocity(delta_vel, delta_vel_dt)) {
        return;
    }

    // integrate delta velocity in earth frame
    const Matrix3f &rotMat = plopter.ahrs.get_rotation_body_to_ned();
    ...
    if (last_flow_ms == 0) {
        // just starting up
        last_flow_ms = plopter.optflow.last_update();
        delta_velocity_ne.zero();
        height_offset = 0;
        return;
    }

    if (plopter.optflow.last_update() == last_flow_ms) {
        // no new flow data
        return;
    }

    // convert delta velocity back to body frame to match the flow sensor
    Vector2f delta_vel_bf = plopter.ahrs.earth_to_body2D(delta_velocity_ne);
    ...
    Vector2f flow_rate_rps = plopter.optflow.flowRate() - plopter.optflow.bodyRate();

    uint32_t dt_ms = plopter.optflow.last_update() - last_flow_ms;
    if (dt_ms > 500) {
