...
void Plopter::ekf_check()
{
    ...
    bool ekf_check_disabled = !plopter.arming.is_armed() || (g2.fs_ekf_thresh <= 0.0f);
#if HAL_QUADPLANE_ENABLED
    if (!quadplopter.in_vtol_posvel_mode()) {
...
bool Plopter::ekf_over_threshold()
{
...
void Plopter::failsafe_ekf_event()
{
    ...
    if (quadplopter.in_vtol_auto()) {
        ...
        plopter.set_mode(mode_qland, ModeReason::EKF_FAILSAFE);
    } else {
        // the pilot is controlling via sticks so fallbacl to QHOVER
        plopter.set_mode(mode_qhover, ModeReason::EKF_FAILSAFE);
    }
#endif
}

// failsafe_ekf_off_event - actions to take when EKF failsafe is cleared
void Plopter::failsafe_ekf_off_event(void)
{
