 ...
 *          FlipState::Start (while plopter is leaning <45deg) : roll right at 400deg/sec, increase throttle
 *          FlipState::Roll (while plopter is between +45deg ~ -90) : roll right at 400deg/sec, reduce throttle
 *          FlipState::Recover (while plopter is between -90deg and original target angle) : use earth frame angle controller to return vehicle to original attitude
 */
...
bool ModeFlip::init(bool ignore_checks)
{
    ...
    if (!plopter.flightmode->allows_flip()) {
        return false;
    }

    // if in acro or stabilize ensure throttle is above zero
    if (plopter.ap.throttle_zero && (plopter.flightmode->mode_number() == Mode::Number::ACRO || plopter.flightmode->mode_number() == Mode::Number::STABILIZE)) {
        return false;
    }
    ...
    if (!motors->armed() || plopter.ap.land_complete) {
        return false;
    }

    // capture original flight mode so that we can return to it after completion
    orig_control_mode = plopter.flightmode->mode_number();
    ...
    const float angle_max = plopter.aparm.angle_max;
...
void ModeFlip::run()
{
    ...
    case FlipState::Recover: {
        ...
        if (fabsf(recovery_angle) <= FLIP_RECOVERY_ANGLE) {
            ...
            if (!plopter.set_mode(orig_control_mode, ModeReason::FLIP_COMPLETE)) {
                // this should never happen but just in case
                plopter.set_mode(Mode::Number::STABILIZE, ModeReason::UNKNOWN);
            }
