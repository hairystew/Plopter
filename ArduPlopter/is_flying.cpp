...
*/
void Plopter::update_is_flying_5Hz(void)
{
    float aspeed=0;
    ...
    is_flying_bool = quadplopter.is_flying();
#endif
    if (is_flying_bool) {
 ...
 */
bool Plopter::is_flying(void)
{
    if (hal.util->get_soft_armed()) {
#if HAL_QUADPLANE_ENABLED
        if (quadplopter.is_flying_vtol()) {
            return true;
        }
#endif
 ...
 */
void Plopter::crash_detection_update(void)
{
    ...
    if (!is_flying() && arming.is_armed())
    {
        if (landing.is_expecting_impact()) {
            ...
            if (!crash_state.checkedHardLanding && // only check once
                ...
                // event when, for example, a minute after a good landing you pick the plopter up and
                // this logic is still running and detects the plopter is on its side as you carry it.
            ...
            case AP_Vehicle::FixedWing::FLIGHT_TAKEOFF:
                if (g.takeoff_throttle_min_accel > 0 &&
                    ...
                    // plopter or it was a failed launch.
                    crashed = true;
                    crash_state.debounce_time_total_ms = CRASH_DETECTION_DELAY_MS;
                }
 ...
 */
bool Plopter::in_preLaunch_flight_stage(void)
{
    if (control_mode == &mode_takeoff && throttle_suppressed) {
        return true;
    }
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.is_vtol_takeoff(mission.get_current_nav_cmd().id)) {
        return false;
    }
#endif
