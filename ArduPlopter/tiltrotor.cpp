...
const AP_Param::GroupInfo Tiltrotor::var_info[] = {
    ...
    // @Description: This is the maximum angle of the tiltable motors at which multicopter control will be enabled. Beyond this angle the plopter will fly solely as a fixed wing aircraft and the motors will tilt to their maximum angle at the TILT_RATE
    // @Units: deg
    // @Increment: 1
    // @Range: 20 80
    // @User: Standard
    ...
    // @Description: This is the angle of the tilt servos when in VTOL mode and at minimum output. This needs to be set for Q_TILT_TYPE=3 to enable vectored control for yaw of tricopter tilt quadplopters. This is also used to limit the forwards travel of bicopter tilts when in VTOL modes
    // @Range: 0 30
    ...
    // @Description: This is the gain for use of tilting motors in fixed wing flight for tilt vectored quadplopters
    // @Range: 0 1
    // @User: Standard
...
Tiltrotor::Tiltrotor(QuadPlopter& _quadplopter, AP_MotorsMulticopter*& _motors):quadplopter(_quadplopter),motors(_motors)
{
...
void Tiltrotor::setup()
{
    ...
    if (quadplopter.motors_var_info == AP_MotorsMatrix::var_info && _is_vectored) {
    ...
    transition = new Tiltrotor_Transition(quadplopter, motors, *this);
    if (!transition) {
...
float Tiltrotor::tilt_max_change(bool up, bool in_flap_range) const
{
    ...
    if (type != TILT_TYPE_BINARY && !up && !in_flap_range) {
        ...
        if (hal.util->get_soft_armed() && !quadplopter.in_vtol_mode() && !quadplopter.assisted_flight) {
            fast_tilt = true;
        }
        if (fast_tilt) {
    ...
    return rate * plopter.G_Dt * (1/90.0);
}
...
void Tiltrotor::continuous_update(void)
{
    ...
    if (!quadplopter.in_vtol_mode() && (!hal.util->get_soft_armed() || !quadplopter.assisted_flight)) {
        ...
        if (!quadplopter.motor_test.running) {
            // the motors are all the way forward, start using them for fwd thrust
            uint8_t mask = is_zero(current_throttle)?0:(uint8_t)tilt_mask.get();
            motors->output_motor_mask(current_throttle, mask, plopter.rudder_dt);
        }
        return;
    }
    ...
    */
    ...
    if (quadplopter.assisted_flight &&
       ...
       float settilt = constrain_float((SRV_Channels::get_output_scaled(SRV_Channel::k_throttle)-MAX(plopter.aparm.throttle_min.get(),0)) * 0.02, 0, 1);
...
void Tiltrotor::binary_update(void)
{
    ...
    if (!quadplopter.in_vtol_mode()) {
        ...
        if (current_tilt >= 1) {
            ...
            // the motors are all the way forward, start using them for fwd thrust
            motors->output_motor_mask(new_throttle, mask, plopter.rudder_dt);
        }
    } else {
        binary_slew(false);
    }
...
void Tiltrotor::vectoring(void)
{
    ...
    if (!hal.util->get_soft_armed() && (plopter.quadplopter.options & QuadPlopter::OPTION_DISARMED_TILT)) {
        ...
        if ((now - hal.util->get_last_armed_change()) > TILT_DELAY_MS) {
            if (quadplopter.in_vtol_mode()) {
                float yaw_out = plopter.channel_rudder->get_control_in();
                yaw_out /= plopter.channel_rudder->get_range();
...
{
    if (type != TILT_TYPE_BICOPTER || quadplopter.motor_test.running) {
        // don't override motor test with motors_output
        return;
    }

    if (!quadplopter.in_vtol_mode() && fully_fwd()) {
    ...
    if (quadplopter.assisted_flight) {
        quadplopter.hold_stabilize(throttle * 0.01f);
...
/*
  when doing a forward transition of a tilt-vectored quadplopter we use
...
void Tiltrotor::update_yaw_target(void)
{
    ...
    if (have_airspeed && labs(plopter.nav_roll_cd)>1000) {
        float dt = (now - transition_yaw_set_ms) * 0.001;
        // calculate the yaw rate to achieve the desired turn rate
        const float airspeed_min = MAX(plopter.aparm.airspeed_min,5);
        const float yaw_rate_cds = fixedwing_turn_rate(plopter.nav_roll_cd*0.01, MAX(aspeed,airspeed_min))*100;
