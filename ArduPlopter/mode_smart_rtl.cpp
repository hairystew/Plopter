 ...
 * Once the plopter is close to home, it will run a standard land controller.
 */
...
void ModeSmartRTL::pre_land_position_run()
{
    ...
    if (wp_nav->reached_wp_destination()) {
        ...
        if (g.rtl_alt_final <= 0 || plopter.failsafe.radio) {
            land_start();
            smart_rtl_state = SubMode::LAND;
        } else {
            set_descent_target_alt(plopter.g.rtl_alt_final);
            descent_start();
...
{
    const bool should_save_position = motors->armed() && (plopter.flightmode->mode_number() != Mode::Number::SMART_RTL);

    plopter.g2.smart_rtl.update(plopter.position_ok(), should_save_position);
}
