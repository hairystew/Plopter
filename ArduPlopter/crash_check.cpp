...
// Code to detect a crash main ArduPlopter code
...
// Code to detect a thrust loss main ArduPlopter code
...
void Plopter::crash_check()
{
    ...
    if (crash_counter >= (CRASH_CHECK_TRIGGER_SEC * scheduler.get_loop_rate_hz())) {
        ...
        // disarm motors
        plopter.arming.disarm(AP_Arming::Method::CRASH);
    }
}

// check for loss of thrust and trigger thrust boost in motors library
void Plopter::thrust_loss_check()
{
    ...
    if ((plopter.g2.flight_options & uint32_t(FlightOptions::DISABLE_THRUST_LOSS_CHECK)) != 0) {
        return;
    }
    ...
    if (thrust_loss_counter >= (THRUST_LOSS_CHECK_TRIGGER_SEC * scheduler.get_loop_rate_hz())) {
        ...
        if ((plopter.g2.flight_options & uint32_t(FlightOptions::RELEASE_GRIPPER_ON_THRUST_LOSS)) != 0) {
...
void Plopter::yaw_imbalance_check()
{
    // no-op if suppresed by flight options param
    if ((plopter.g2.flight_options & uint32_t(FlightOptions::DISABLE_YAW_IMBALANCE_WARNING)) != 0) {
        return;
    }
...
void Plopter::parachute_check()
{
    ...
    if (parachute.release_initiated()) {
        plopter.arming.disarm(AP_Arming::Method::PARACHUTE_RELEASE);
        return;
    }
...
void Plopter::parachute_release()
{
    // disarm motors
...
void Plopter::parachute_manual_release()
{
    // exit immediately if parachute is not enabled
    if (!parachute.enabled()) {
        return;
