...
* Throttle slew limit
*****************************************/
void Plopter::throttle_slew_limit(SRV_Channel::Aux_servo_function_t func)
{
#if HAL_QUADPLANE_ENABLED
    const bool do_throttle_slew = (control_mode->does_auto_throttle() || quadplopter.in_assisted_flight() || quadplopter.in_vtol_mode());
#else
...
#if HAL_QUADPLANE_ENABLED
    if (g.takeoff_throttle_slewrate != 0 && quadplopter.in_transition()) {
        slewrate = g.takeoff_throttle_slewrate;
    }
#endif
...
*/
bool Plopter::suppress_throttle(void)
{
...
/*
  ...
  SERVOn_* parameters
 */
void Plopter::channel_function_mixer(SRV_Channel::Aux_servo_function_t func1_in, SRV_Channel::Aux_servo_function_t func2_in,
 ...
 */
void Plopter::dspoiler_update(void)
{
    ...
    if (crow_mode == Plopter::CrowMode::CROW_DISABLED) {   //override totally aileron crow if crow RC switch set to disabled
        weight_outer = 0;
    }
 ...
 */
void Plopter::airbrake_update(void)
{
 ...
 */
void Plopter::set_servos_idle(void)
{
    int16_t servo_value;
 ...
 */
void Plopter::set_servos_manual_passthrough(void)
{
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.available() && (quadplopter.options & QuadPlopter::OPTION_IDLE_GOV_MANUAL)) {
        // for quadplopters it can be useful to run the idle governor in MANUAL mode
 ...
 */
void Plopter::throttle_voltage_comp(int8_t &min_throttle, int8_t &max_throttle) const
{
 ...
 */
void Plopter::throttle_watt_limiter(int8_t &min_throttle, int8_t &max_throttle)
{
    uint32_t now = millis();
 ...
 */
void Plopter::set_servos_controlled(void)
{
    ...
    if (!hal.util->get_soft_armed()) {
    ...
    } else if (plopter.nav_scripting.current_ms > 0 && nav_scripting.enabled) {
        ...
        SRV_Channels::set_output_scaled(SRV_Channel::k_throttle, get_throttle_input(true));
#if HAL_QUADPLANE_ENABLED
    } else if (quadplopter.in_vtol_mode()) {
        float fwd_thr = 0;
        // if armed and not spooled down ask quadplopter code for forward throttle
        if (quadplopter.motors->armed() &&
            quadplopter.motors->get_desired_spool_state() != AP_Motors::DesiredSpoolState::SHUT_DOWN) {

            fwd_thr = constrain_float(quadplopter.forward_throttle_pct(), min_throttle, max_throttle);
        }
    ...
    if (!is_flying() && arming.is_armed()) {
        ...
        if (throw_detected || throttle_up_detected) {
            plopter.ahrs.set_takeoff_expected(true);
        }
    }
}
...
/*
  setup flap outputs
 */
void Plopter::set_servos_flaps(void)
{
 ...
 */
void Plopter::set_landing_gear(void)
{
...
/*
  support for twin-engine plopters
 */
void Plopter::servos_twin_engine_mix(void)
{
    float throttle = SRV_Channels::get_output_scaled(SRV_Channel::k_throttle);
    float rud_gain = float(plopter.g2.rudd_dt_gain) * 0.01f;
...
*/
void Plopter::force_flare(void)
{
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_transition() && plopter.arming.is_armed()) { //allows for ground checking of flare tilts
        return;
    }
    ...
    */
    if (!control_mode->does_auto_throttle() && flare_mode != FlareMode::FLARE_DISABLED && throttle_at_zero()) {
        ...
        if (quadplopter.tiltrotor.enabled() && (quadplopter.tiltrotor.type == Tiltrotor::TILT_TYPE_BICOPTER)) {
...
void Plopter::set_servos(void)
{
    ...
    // do any transition updates for quadplopter
#if HAL_QUADPLANE_ENABLED
    quadplopter.update();
#endif
...
*/
void Plopter::landing_neutral_control_surface_servos(void)
{
...
/*
  ...
  hal.rcout
 */
void Plopter::servos_output(void)
{
    SRV_Channels::cork();
    ...
    quadplopter.tailsitter.output();
    quadplopter.tiltrotor.bicopter_output();
#endif
...
void Plopter::update_throttle_hover() {
    // update hover throttle at 100Hz
#if HAL_QUADPLANE_ENABLED
    quadplopter.update_throttle_hover();
#endif
}
 ...
 */
void Plopter::servos_auto_trim(void)
{
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.in_assisted_flight() || quadplopter.in_vtol_mode()) {
        // can't auto-trim with quadplopter motors running
        return;
    }
#endif
