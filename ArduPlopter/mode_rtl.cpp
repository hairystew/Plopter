...
bool ModeRTL::init(bool ignore_checks)
{
    ...
    terrain_following_allowed = !plopter.failsafe.terrain;
...
void ModeRTL::run(bool disarm_on_land)
{
    ...
    if (_state_complete) {
        ...
        case SubMode::LOITER_AT_HOME:
            if (rtl_path.land || plopter.failsafe.radio) {
                land_start();
            }else{
                descent_start();
            }
...
void ModeRTL::climb_start()
{
    ...
    if (!wp_nav->set_wp_destination_loc(rtl_path.climb_target) || !wp_nav->set_wp_destination_next_loc(rtl_path.return_target)) {
        ...
        plopter.set_mode(Mode::Number::LAND, ModeReason::TERRAIN_FAILSAFE);
        return;
    }
...
void ModeRTL::loiterathome_run()
{
    ...
    if ((millis() - _loiter_start_time) >= (uint32_t)g.rtl_loiter_time.get()) {
        if (auto_yaw.mode() == AUTO_YAW_RESETTOARMEDYAW) {
            ...
            if (abs(wrap_180_cd(ahrs.yaw_sensor-plopter.initial_armed_bearing)) <= 200) {
                _state_complete = true;
            }
        } else {
...
void ModeRTL::descent_run()
{
    ...
    if (!plopter.failsafe.radio) {
        if ((g.throttle_behavior & THR_BEHAVE_HIGH_THROTTLE_CANCELS_LAND) != 0 && plopter.rc_throttle_control_in_filter.get() > LAND_CANCEL_TRIGGER_THR){
            ...
            if (!plopter.set_mode(Mode::Number::LOITER, ModeReason::THROTTLE_LAND_ESCAPE)) {
                plopter.set_mode(Mode::Number::ALT_HOLD, ModeReason::THROTTLE_LAND_ESCAPE);
            }
        }
    ...
    _state_complete = labs(rtl_path.descent_target.alt - plopter.current_loc.alt) < 20;
}
...
{
    // check if we've completed this stage of RTL
    _state_complete = plopter.ap.land_complete;

    // disarm when the landing detector says we've landed
    if (disarm_on_land && plopter.ap.land_complete && motors->get_spool_state() == AP_Motors::SpoolState::GROUND_IDLE) {
...
void ModeRTL::compute_return_target()
{
...
#if AC_RALLY == ENABLED
    rtl_path.return_target = plopter.rally.calc_best_rally_or_home_location(plopter.current_loc, ahrs.get_home().alt);
#else
    rtl_path.return_target = ahrs.get_home();
#endif

    // curr_alt is current altitude above home or above terrain depending upon use_terrain
    int32_t curr_alt = plopter.current_loc.alt;
    ...
    if (alt_type == ReturnTargetAltType::RANGEFINDER) {
        if (plopter.get_rangefinder_height_interpolated_cm(curr_alt)) {
    ...
    if (alt_type == ReturnTargetAltType::TERRAINDATABASE) {
        ...
        if (plopter.current_loc.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, curr_terr_alt) &&
    ...
    if ((plopter.fence.get_enabled_fences() & AC_FENCE_TYPE_ALT_MAX) != 0) {
        // get return target as alt-above-home so it can be compared to fence's alt
        if (rtl_path.return_target.get_alt_cm(Location::AltFrame::ABOVE_HOME, target_alt)) {
            float fence_alt = plopter.fence.get_safe_alt_max()*100.0f;
...
{
    return (plopter.g2.rtl_options.get() & uint32_t(Options::IgnorePilotYaw)) == 0;
}
