...
const AP_Param::GroupInfo Tailsitter::var_info[] = {
    ...
    // @Description: This controls whether stick input when hovering as a tailsitter follows the conventions for fixed wing hovering or multicopter hovering. When PlopterMode is not enabled (bit0 = 0) the roll stick will roll the aircraft in earth frame and yaw stick will yaw in earth frame. When PlopterMode input is enabled, the roll and yaw sticks are swapped so that the roll stick controls earth-frame yaw and rudder controls earth-frame roll. When body-frame roll is enabled (bit1 = 1), the yaw stick controls earth-frame yaw rate and the roll stick controls roll in the tailsitter's body frame when flying level.
    // @Bitmask: 0:PlopterMode,1:BodyFrameRoll
...
Tailsitter::Tailsitter(QuadPlopter& _quadplopter, AP_MotorsMulticopter*& _motors):quadplopter(_quadplopter),motors(_motors)
{
...
void Tailsitter::setup()
{
    ...
    if (!enable.configured() && (((quadplopter.frame_class == AP_Motors::MOTOR_FRAME_TAILSITTER) || (motor_mask != 0)) && (quadplopter.tiltrotor.type != Tiltrotor::TILT_TYPE_BICOPTER))) {
    ...
    if (!transition_rate_fw.configured()) {
        transition_rate_fw.set_and_save(transition_angle_fw / (quadplopter.transition_time_ms/2000.0f));
    }
    ...
    _is_vectored = (quadplopter.frame_class == AP_Motors::MOTOR_FRAME_TAILSITTER) &&
    ...
    if (quadplopter.frame_class == AP_Motors::MOTOR_FRAME_TAILSITTER) {
    ...
    if (enable == 2) {
        quadplopter.q_assist_state = QuadPlopter::Q_ASSIST_STATE_ENUM::Q_ASSIST_FORCE;
        quadplopter.air_mode = AirMode::ASSISTED_FLIGHT_ONLY;
        ...
        quadplopter.options.set(quadplopter.options.get() | QuadPlopter::OPTION_ONLY_ARM_IN_QMODE_OR_AUTO);
    }

    transition = new Tailsitter_Transition(quadplopter, motors, *this);
    if (!transition) {
        AP_BoardConfig::allocation_error("tailsitter transition");
    }
    quadplopter.transition = transition;
...
{
    return quadplopter.frame_class == AP_Motors::MOTOR_FRAME_TAILSITTER
...
{
    if (!enabled() || quadplopter.motor_test.running || !quadplopter.initialised) {
    ...
    if (!active() || in_vtol_transition()) {
        ...
        if (hal.util->get_soft_armed() && in_vtol_transition() && !quadplopter.throttle_wait && quadplopter.is_flying()) {
            /*
            ...
            */
            if (!is_negative(transition_throttle_vtol)) { 
                ...
                throttle = MAX(throttle,motors->actuator_to_thrust(plopter.aparm.throttle_cruise.get() * 0.01));
            }
            ...
            if (!quadplopter.assisted_flight) {
                ...
                quadplopter.attitude_control->set_throttle_out(throttle, false, 0);
        ...
        if (!quadplopter.assisted_flight) {
            ...
            motors->output_motor_mask(throttle, motor_mask, plopter.rudder_dt);

            // in forward flight: set motor tilt servos and throttles using FW controller
            if (vectored_forward_gain > 0) {
                // remove scaling from surface speed scaling and apply throttle scaling
                const float scaler = plopter.control_mode == &plopter.mode_manual?1:(quadplopter.FW_vector_throttle_scaling() / plopter.get_speed_scaler());
    ...
    // to motors_output() from quadplopter.update(), unless we are in assisted flight
    // tailsitter in TRANSITION_ANGLE_WAIT_FW is not really in assisted flight, its still in a VTOL mode
    if (quadplopter.assisted_flight && (transition->transition_state != Tailsitter_Transition::TRANSITION_ANGLE_WAIT_FW)) {
        quadplopter.hold_stabilize(throttle);
        quadplopter.motors_output(true);

        if ((quadplopter.options & QuadPlopter::OPTION_TAILSIT_Q_ASSIST_MOTORS_ONLY) != 0) {
            // only use motors for Q assist, control surfaces remain under plopter control
            // zero copter I terms and use plopter
            quadplopter.attitude_control->reset_rate_controller_I_terms();
            ...
            // skip remainder of the function that overwrites plopter control surface outputs with copter
            return;
        }
    } else {
        quadplopter.motors_output(false);
    }

    // In full Q assist it is better to use copter I and zero plopter
    plopter.pitchController.reset_I();
    plopter.rollController.reset_I();
    plopter.yawController.reset_I();
    ...
    if (vectored_hover_gain > 0) {
         ...
         */
        float des_pitch_cd = quadplopter.attitude_control->get_att_target_euler_cd().y;
        int32_t pitch_error_cd = (des_pitch_cd - quadplopter.ahrs_view->pitch_sensor) * 0.5;
        float extra_pitch = constrain_float(pitch_error_cd, -SERVO_MAX, SERVO_MAX) / SERVO_MAX;
        float extra_sign = extra_pitch > 0?1:-1;
        float extra_elevator = 0;
        if (!is_zero(extra_pitch) && quadplopter.in_vtol_mode()) {
    ...
    float elevator_mix = SRV_Channels::get_output_scaled(SRV_Channel::k_elevator) * (100.0 - plopter.g.mixing_offset) * 0.01 * plopter.g.mixing_gain;
    float aileron_mix = SRV_Channels::get_output_scaled(SRV_Channel::k_aileron) * (100.0 + plopter.g.mixing_offset) * 0.01 * plopter.g.mixing_gain;
    float rudder_mix = SRV_Channels::get_output_scaled(SRV_Channel::k_rudder) * (100.0 + plopter.g.mixing_offset) * 0.01 * plopter.g.mixing_gain;
...
bool Tailsitter::transition_fw_complete(void)
{
    ...
    if (labs(quadplopter.ahrs_view->pitch_sensor) > transition_angle_fw*100) {
        gcs().send_text(MAV_SEVERITY_INFO, "Transition FW done");
        return true;
    }
    if (labs(quadplopter.ahrs_view->roll_sensor) > MAX(4500, plopter.roll_limit_cd + 500)) {
...
void Tailsitter::check_input(void)
{
    if (active() && (input_type & TAILSITTER_INPUT_PLANE)) {
        ...
        int16_t roll_in = plopter.channel_roll->get_control_in();
        int16_t yaw_in = plopter.channel_rudder->get_control_in();
        plopter.channel_roll->set_control_in(yaw_in);
        plopter.channel_rudder->set_control_in(-roll_in);
    }
}
...
{
    if (!enabled() || !quadplopter.in_vtol_mode()) {
        return false;
    }
...
{
    return enabled() && !quadplopter.in_vtol_mode() && transition->transition_state == Tailsitter_Transition::TRANSITION_DONE;
}
...
void Tailsitter::speed_scaling(void)
{
    ...
    if ((gain_scaling_mask & TAILSITTER_GSCL_ATT_THR) != 0) {
        ...
        const float c_tilt = quadplopter.ahrs_view->get_rotation_body_to_ned().c.z;
        ...
        const float posdelta = plopter.G_Dt / posTC;
        const float negdelta = plopter.G_Dt / negTC;
        ...
        float airspeed;
        if (!quadplopter.ahrs.airspeed_estimate(airspeed)) {
            ...
            const float rho = SSL_AIR_DENSITY * plopter.barometer.get_air_density_ratio();
            float hover_rho = rho;
            ...
            if (is_positive(disk_loading_min_outflow)) {
                ...
                if (is_positive(airspeed)) {
                    ...
                    Vector3f vel;
                    if (quadplopter.ahrs.get_velocity_NED(vel)) {
                        reverse_airspeed = quadplopter.ahrs.earth_to_body(vel - quadplopter.ahrs.wind_estimate()).x;
                    }
    ...
    if ((gain_scaling_mask & TAILSITTER_GSCL_ALTITUDE) != 0) {
        ...
        spd_scaler /= plopter.barometer.get_air_density_ratio();
    }
...
void Tailsitter_Transition::update()
{
    ...
    quadplopter.assisted_flight = quadplopter.should_assist(aspeed, have_airspeed);
    ...
    case TRANSITION_ANGLE_WAIT_FW: {
        if (tailsitter.transition_fw_complete()) {
            ...
            if (hal.util->get_soft_armed()) {
                ...
                fw_limit_initial_pitch = constrain_float(quadplopter.ahrs.pitch_sensor,-8500,8500);
                plopter.nav_pitch_cd = fw_limit_initial_pitch;
        ...
        plopter.nav_pitch_cd = constrain_float(fw_transition_initial_pitch - (quadplopter.tailsitter.transition_rate_fw * dt) * 0.1f * (plopter.fly_inverted()?-1.0f:1.0f), -8500, 8500);
        plopter.nav_roll_cd = 0;
        quadplopter.attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(plopter.nav_roll_cd,
                                                                      plopter.nav_pitch_cd,
                                                                      0);
        // set throttle at either hover throttle or current throttle, whichever is higher, through the transition
        quadplopter.attitude_control->set_throttle_out(MAX(motors->get_throttle_hover(),quadplopter.attitude_control->get_throttle_in()), true, 0);
...
void Tailsitter_Transition::VTOL_update()
{
    ...
    if (transition_state == TRANSITION_ANGLE_WAIT_VTOL) {
        ...
        quadplopter.assisted_flight = quadplopter.should_assist(aspeed, have_airspeed);
        if (!quadplopter.tailsitter.transition_vtol_complete()) {
            return;
        }
        // transition to VTOL complete, if armed set vtol rate limit starting point
        if (hal.util->get_soft_armed()) {
            vtol_limit_start_ms = now;
            vtol_limit_initial_pitch = quadplopter.ahrs_view->pitch_sensor;
        }
    }
    restart();
}
...
{
    bool show_vtol = quadplopter.in_vtol_mode();
...
void Tailsitter_Transition::set_FW_roll_pitch(int32_t& nav_pitch_cd, int32_t& nav_roll_cd, bool& allow_stick_mixing)
{
    ...
    if (tailsitter.in_vtol_transition(now)) {
        ...
        vtol_transition_initial_pitch = constrain_float(plopter.nav_pitch_cd,-8500,8500);
...
void Tailsitter_Transition::restart()
{
    ...
    fw_transition_initial_pitch = constrain_float(quadplopter.attitude_control->get_attitude_target_quat().get_euler_pitch() * degrees(100.0),-8500,8500);
}
