...
{
    return get_bearing_cd(plopter.inertial_nav.get_position_xy_cm(), roi.xy());
}

float Mode::AutoYaw::look_ahead_yaw()
{
    const Vector3f& vel = plopter.inertial_nav.get_velocity_neu_cms();
    const float speed_sq = vel.xy().length_squared();
    // Commanded Yaw to automatically look ahead.
    if (plopter.position_ok() && (speed_sq > (YAW_LOOK_AHEAD_MIN_SPEED * YAW_LOOK_AHEAD_MIN_SPEED))) {
...
{
    switch (plopter.g.wp_yaw_behavior) {
...
void Mode::AutoYaw::set_mode(autopilot_yaw_mode yaw_mode)
{
    ...
    case AUTO_YAW_LOOK_AHEAD:
        // Commanded Yaw to automatically look ahead.
        _look_ahead_yaw = plopter.ahrs.yaw_sensor;
        break;
...
void Mode::AutoYaw::set_fixed_yaw(float angle_deg, float turn_rate_ds, int8_t direction, bool relative_angle)
{
    ...
    _yaw_angle_cd = plopter.attitude_control->get_att_target_euler_cd().z;
    _yaw_rate_cds = 0.0;
    ...
    if (!is_positive(turn_rate_ds)) {
        ...
        _fixed_yaw_slewrate_cds = plopter.attitude_control->get_slew_yaw_max_degs() * 100.0;
    } else {
        _fixed_yaw_slewrate_cds = MIN(plopter.attitude_control->get_slew_yaw_max_degs(), turn_rate_ds) * 100.0;
    }
...
void Mode::AutoYaw::set_roi(const Location &roi_location)
{
    ...
    if (roi_location.alt == 0 && roi_location.lat == 0 && roi_location.lng == 0) {
        ...
        if (plopter.camera_mount.get_mode() == MAV_MOUNT_MODE_GPS_POINT) {
        ...
        if (!plopter.camera_mount.has_pan_control()) {
            if (roi_location.get_vector_from_origin_NEU(roi)) {
                auto_yaw.set_mode(AUTO_YAW_ROI);
            }
        }
        // send the command to the camera mount
        plopter.camera_mount.set_roi_target(roi_location);
...
float Mode::AutoYaw::yaw()
{
    ...
    case AUTO_YAW_RESETTOARMEDYAW:
        ...
        return plopter.initial_armed_bearing;

    case AUTO_YAW_CIRCLE:
#if MODE_CIRCLE_ENABLED
        if (plopter.circle_nav->is_active()) {
            return plopter.circle_nav->get_yaw();
        }
#endif
        // return the current attitude target
        return wrap_360_cd(plopter.attitude_control->get_att_target_euler_cd().z);
    ...
    default:
        ...
        // we don't use wp_bearing because we don't want the plopter to turn too much during flight
        return plopter.pos_control->get_yaw_cd();
    }
}
...
float Mode::AutoYaw::rate_cds() const
{
    ...
    case AUTO_YAW_LOOK_AT_NEXT_WP:
        return plopter.pos_control->get_yaw_rate_cds();
    }
