...
void Plopter::update_throttle_hover()
{
    ...
    if (flightmode->has_manual_throttle() || (plopter.flightmode->mode_number() == Mode::Number::DRIFT)) {
        return;
    }
...
float Plopter::get_pilot_desired_climb_rate(float throttle_control)
{
...
float Plopter::get_non_takeoff_throttle()
{
    return MAX(0,motors->get_throttle_hover()/2.0f);
}

// set_accel_throttle_I_from_pilot_throttle - smoothes transition from pilot controlled throttle to autopilot throttle
void Plopter::set_accel_throttle_I_from_pilot_throttle()
{
...
void Plopter::rotate_body_frame_to_NE(float &x, float &y)
{
...
uint16_t Plopter::get_pilot_speed_dn() const
{
