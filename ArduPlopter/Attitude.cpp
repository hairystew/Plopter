 ...
 */
float Plopter::calc_speed_scaler(void)
{
    ...
    if (ahrs.airspeed_estimate(aspeed)) {
        ...
        if (quadplopter.in_vtol_mode() && hal.util->get_soft_armed()) {
    ...
    if (!plopter.ahrs.airspeed_sensor_enabled()  &&
        (plopter.g2.flight_options & FlightOptions::SURPRESS_TKOFF_SCALING) &&
        (plopter.flight_stage == AP_Vehicle::FixedWing::FLIGHT_TAKEOFF)) { //scaling is surpressed during climb phase of automatic takeoffs with no airspeed sensor being used due to problems with inaccurate airspeed estimates
 ...
 */
bool Plopter::stick_mixing_enabled(void)
{
    ...
    if (control_mode == &mode_qrtl &&
        quadplopter.poscontrol.get_state() >= QuadPlopter::QPOS_POSITION1) {
        // user may be repositioning
        return false;
    }
    if (quadplopter.in_vtol_land_poscontrol()) {
        // user may be repositioning
        return false;
    }
#endif
    if (control_mode->does_auto_throttle() && plopter.control_mode->does_auto_navigation()) {
...
/*
  ...
  stabilize the plopter at the given roll
 */
void Plopter::stabilize_roll(float speed_scaler)
{
    if (fly_inverted()) {
...
float Plopter::stabilize_roll_get_roll_out(float speed_scaler)
{
#if HAL_QUADPLANE_ENABLED
    if (!quadplopter.use_fw_attitude_controllers()) {
        // use the VTOL rate for control, to ensure consistency
        const auto &pid_info = quadplopter.attitude_control->get_rate_roll_pid().get_pid_info();
    ...
    return rollController.get_servo_out(nav_roll_cd - ahrs.roll_sensor, speed_scaler, disable_integrator,
                                        ground_mode && !(plopter.g2.flight_options & FlightOptions::DISABLE_GROUND_PID_SUPPRESSION));
}

/*
  this is the main pitch stabilization function. It takes the
  previously set nav_pitch and calculates servo_out values to try to
  stabilize the plopter at the given attitude.
 */
void Plopter::stabilize_pitch(float speed_scaler)
{
...
float Plopter::stabilize_pitch_get_pitch_out(float speed_scaler)
{
#if HAL_QUADPLANE_ENABLED
    if (!quadplopter.use_fw_attitude_controllers()) {
        // use the VTOL rate for control, to ensure consistency
        const auto &pid_info = quadplopter.attitude_control->get_rate_pitch_pid().get_pid_info();
...
#if HAL_QUADPLANE_ENABLED
    const bool quadplopter_in_transition = quadplopter.in_transition();
#else
    const bool quadplopter_in_transition = false;
#endif
 ...
 */
void Plopter::stabilize_stick_mixing_direct()
{
    ...
    if ((control_mode == &mode_loiter) && (plopter.g2.flight_options & FlightOptions::ENABLE_LOITER_ALT_CONTROL)) {
 ...
 */
void Plopter::stabilize_stick_mixing_fbw()
{
    if (!stick_mixing_enabled() ||
        control_mode == &mode_acro ||
        control_mode == &mode_fbwa ||
    ...
    // the user can direct the plopter in any direction with stick
    // mixing.
 ...
 */
void Plopter::stabilize_yaw(float speed_scaler)
{
 ...
 */
void Plopter::stabilize_training(float speed_scaler)
{
 ...
 */
void Plopter::stabilize_acro(float speed_scaler)
{
    ...
    } else if (plopter.g2.flight_options & FlightOptions::ACRO_YAW_DAMPER) {
        // use yaw controller
        calc_nav_yaw_coordinated(speed_scaler);
    } else {
        /*
...
void Plopter::stabilize()
{
    ...
    if (quadplopter.available()) {
        quadplopter.transition->set_FW_roll_pitch(nav_pitch_cd, nav_roll_cd, allow_stick_mixing);
    }
#endif
...
#if HAL_QUADPLANE_ENABLED
    } else if (control_mode->is_vtol_mode() && !quadplopter.tailsitter.in_vtol_transition(now)) {
        // run controlers specific to this mode
        plopter.control_mode->run();

        // we also stabilize using fixed wing surfaces
        if (plopter.control_mode->mode_number() == Mode::Number::QACRO) {
            plopter.stabilize_acro(speed_scaler);
        } else {
            plopter.stabilize_roll(speed_scaler);
            plopter.stabilize_pitch(speed_scaler);
        }
#endif
    } else {
...
void Plopter::calc_throttle()
{
    ...
    if (control_mode->is_guided_mode() &&
            ...
            millis() - plopter.guided_state.last_forced_throttle_ms < 3000) {
        commanded_throttle = plopter.guided_state.forced_throttle;
    }
 ...
 */
void Plopter::calc_nav_yaw_coordinated(float speed_scaler)
{
    bool disable_integrator = false;
    ...
    if (control_mode->is_guided_mode() &&
            plopter.guided_state.last_forced_rpy_ms.z > 0 &&
            millis() - plopter.guided_state.last_forced_rpy_ms.z < 3000) {
        commanded_rudder = plopter.guided_state.forced_rpy_cd.z;
 ...
 */
void Plopter::calc_nav_yaw_course(void)
{
 ...
 */
void Plopter::calc_nav_yaw_ground(void)
{
 ...
 */
void Plopter::calc_nav_pitch()
{
    // Calculate the Pitch of the plopter
    // --------------------------------
    int32_t commanded_pitch = TECS_controller.get_pitch_demand();

    // Received an external msg that guides roll in the last 3 seconds?
    if (control_mode->is_guided_mode() &&
            plopter.guided_state.last_forced_rpy_ms.y > 0 &&
            millis() - plopter.guided_state.last_forced_rpy_ms.y < 3000) {
        commanded_pitch = plopter.guided_state.forced_rpy_cd.y;
    }
 ...
 */
void Plopter::calc_nav_roll()
{
    int32_t commanded_roll = nav_controller->nav_roll_cd();

    // Received an external msg that guides roll in the last 3 seconds?
    if (control_mode->is_guided_mode() &&
            plopter.guided_state.last_forced_rpy_ms.x > 0 &&
            millis() - plopter.guided_state.last_forced_rpy_ms.x < 3000) {
        commanded_roll = plopter.guided_state.forced_rpy_cd.x;
...
/*
  ...
  keeping up good airspeed in FBWA mode easier, as the plopter will
  automatically pitch down a little when at low throttle. It makes
  FBWA landings without stalling much easier.
 */
void Plopter::adjust_nav_pitch_throttle(void)
{
...
/*
  ...
  airspeed
 */
void Plopter::update_load_factor(void)
{
...
#if HAL_QUADPLANE_ENABLED
    if (quadplopter.available() && quadplopter.transition->set_FW_roll_limit(roll_limit_cd)) {
        nav_roll_cd = constrain_int32(nav_roll_cd, -roll_limit_cd, roll_limit_cd);
        return;
    }
#endif
